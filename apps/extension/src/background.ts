...
async function handleMessage(browser: BrowserApi, message: unknown): Promise<BackgroundResponse> {
  ...
  switch (message.type) {
    ...
    case "saveToken": {
      ...
      if (!token) {
        return { ok: false, error: "missing_token", message: "Paste the pairing token from JobHunter Settings." };
      }
...
async function captureCurrentTab(browser: BrowserApi): Promise<BackgroundResponse> {
  ...
  if (!token) {
    return { ok: false, error: "not_paired", message: "Pair the extension with JobHunter before capturing." };
  }
  const tab = await activeTab(browser);
  if (!tab.id || !isHttpUrl(tab.url)) {
    return { ok: false, error: "unsupported_page", message: "JobHunter can capture only http(s) pages." };
  }
  ...
  try {
    ...
    return {
      ...
      message: error instanceof Error ? error.message : "Local JobHunter API is unavailable.",
    };
  }
}
