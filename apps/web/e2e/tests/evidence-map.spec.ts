...
test("Evidence map: job handoff filters evidence and usage links return to artifacts and jobs", async ({
  ...
  await drawer.getByRole("link", { name: `Open evidence map for ${PLATFORM_JOB_TITLE}` }).click();

  await expect(page).toHaveURL(/\/evidence-map\?.*job=/);
  await expect(page.getByRole("heading", { name: "Career evidence map" })).toBeVisible();
  await expect(page.getByRole("link", { name: "Clear job filter" })).toBeVisible();

  await page
    .getByRole("navigation", { name: "Evidence entries" })
    .getByRole("link", { name: /Owned platform reliability improvements for incident response/i })
    .click();
  await expect(page.getByText("Lead platform reliability improvements across critical services.")).toBeVisible();
  await expect(
  ...
  const artifactUsage = page
    ...
    .filter({ hasText: "Owned platform reliability improvements for incident response." })
    .first();
  await expect(artifactUsage).toHaveAttribute("href", "/artifacts/qa-platform-resume-text");
  ...
  const requirementUsage = page
    .locator(".evidence-usage-link")
    .filter({ hasText: "Lead platform reliability improvements across critical services." })
    .first();
