...
const sampleCompensationSourcePolicy = {
  ...
  sources: [
    ...
    },
    {
      ...
      notes: ["Refresh reads tokenless provider-published Markdown pages and falls back to the same public page's structured data when needed."],
    },
    {
      sourceId: "glassdoor",
      ...
      notes: ["Refresh automatically loads configured permitted rows from JOBCTRL_GLASSDOOR_OBSERVATIONS_PATH or JOBCTRL_GLASSDOOR_OBSERVATIONS_URL when access is permitted."],
    },
  ],
};
...
export const handlers = [
  ...
  http.patch("*/v1/compensation/sources", async ({ request }) => {
    ...
    const ready =
      ...
      (body.sourceId !== "levels_fyi" ||
        body.accessMode === "public_markdown" ||
        body.europeCoverageConfirmed === true);
    return HttpResponse.json({
  ...
  http.post("*/v1/artifacts/:artifactId/open", ({ params }) =>
    HttpResponse.json({
      ...
      opened: true,
      path: `/tmp/jobctrl-test/artifacts/${String(params["artifactId"])}.pdf`,
    }),
  ),
