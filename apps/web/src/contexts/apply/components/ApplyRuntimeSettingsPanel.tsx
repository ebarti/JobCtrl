  ...
  return (
    <section className="card full">
      ...
      <form className="config-form" onSubmit={(event) => { event.preventDefault(); void form.handleSubmit(); }}>
        ...
        <form.Field name="applyMaxBudgetUsd">{(field) => <label className="field" htmlFor="apply-max-budget"><span>Maximum AI budget per application (USD)</span><input id="apply-max-budget" name="applyMaxBudgetUsd" type="number" min={0} step={0.01} readOnly={!budget.editable} value={field.state.value} onChange={(event) => field.handleChange(Number(event.target.value))} /><small>0 is a zero-dollar cap, not unlimited. {policyContext(budget.source)}</small></label>}</form.Field>
        <form.Field name="applyTimeoutSeconds">{(field) => <label className="field" htmlFor="apply-timeout"><span>Apply agent timeout (seconds)</span><input id="apply-timeout" name="applyTimeoutSeconds" type="number" min={60} max={3600} step={1} readOnly={!timeout.editable} value={field.state.value} onChange={(event) => field.handleChange(Number(event.target.value))} /><small>Per application agent; separate from Temporal activity timeouts. {policyContext(timeout.source)}</small></label>}</form.Field>
