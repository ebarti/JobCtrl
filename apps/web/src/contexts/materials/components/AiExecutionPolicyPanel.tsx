  ...
  return (
    <section className="card full">
      ...
      <form className="config-form" onSubmit={(event) => { event.preventDefault(); void form.handleSubmit(); }}>
        ...
        <form.Field name="analysisLegs">{(field) => <fieldset className="field wide checkbox-group-field"><legend>Employer analysis perspectives</legend><div className="checkbox-options">{LEG_OPTIONS.map((option) => <label className="choice target-choice" key={option.value}><input name="analysisLegs" type="checkbox" disabled={!effective.analysisLegs.editable} checked={field.state.value.includes(option.value)} onChange={(event) => { const next = event.target.checked ? [...field.state.value, option.value] : field.state.value.filter((leg) => leg !== option.value); if (next.length) field.handleChange(next); }} /><span>{option.label}</span></label>)}</div><small>{context(effective.analysisLegs.source, "next analysis")}</small></fieldset>}</form.Field>
        ...
        <form.Field name="tailoringJudgeMinScore">{(field) => <label className="field" htmlFor="tailoring-judge-score"><span>Minimum judge score</span><input id="tailoring-judge-score" name="tailoringJudgeMinScore" type="number" min={0} max={1} step={0.01} readOnly={!effective.tailoringJudgeMinScore.editable} value={field.state.value} onChange={(event) => field.handleChange(Number(event.target.value))} /><small>{context(effective.tailoringJudgeMinScore.source, "next tailoring workflow")}</small></label>}</form.Field>
...
function ModelSelectControl({ name, label, models, value, readOnly, help, onChange }: { name: "generatorPrimary" | "generatorFallback" | "tailoringJudgeModel"; label: string; models: Array<{ value: string; label: string }>; value: string; readOnly: boolean; help: string; onChange: (value: string) => void }) {
  ...
  return <label className="field" htmlFor={`ai-policy-${name}`}><span>{label}</span><select id={`ai-policy-${name}`} name={name} disabled={readOnly} value={value} onChange={(event) => onChange(event.target.value)}><option value="">Provider/default policy</option>{options.map((model) => <option key={model.value} value={model.value}>{model.label}</option>)}</select><small>{help}</small></label>;
}
