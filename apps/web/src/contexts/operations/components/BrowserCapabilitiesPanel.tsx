import type { BrowserCapabilityId } from "@jobctrl/contracts";
      ...
      <div className="provider-card-list" aria-busy={query.isPending}>
        {(query.data?.capabilities ?? []).map((capability) => (
          <article className="provider-card" key={capability.id}>
            ...
            {capability.id === "core-browser" ? <p className="provider-copy">Managed by JobCtrl and read-only.</p> : (
              <div className="provider-form">
                <label className="field" htmlFor={`browser-executable-${capability.id}`}>
                  <span>Chrome or Chromium executable path</span>
                  <input id={`browser-executable-${capability.id}`} name={`browser-executable-${capability.id}`} type="password" autoComplete="off" disabled={demo || capability.enabled} value={executablePaths[capability.id] ?? ""} onChange={(event) => setExecutablePaths((current) => ({ ...current, [capability.id]: event.target.value }))} />
                  <small>Write-only: never returned, stored in browser state, or shown after submission.</small>
                </label>
                <div className="form-actions">
                  <button className="tab on" type="button" disabled={demo || capability.enabled || enable.isPending} onClick={() => void enableCapability(capability.id)}>enable selected browser</button>
                  <button className="tab" type="button" disabled={demo || !capability.enabled || disable.isPending} onClick={() => void disable.mutateAsync(capability.id).then(() => setMessage(`${LABELS[capability.id]} disabled immediately.`)).catch(() => setMessage("Capability disable failed."))}>disable now</button>
                </div>
                {capability.id === "authenticated-linkedin-browser" && capability.enabled ? (
