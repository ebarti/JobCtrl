...
export function useUpdateScoringGuidanceMutation(): UseMutationResult<SettingsResponse, Error, SettingsUpdateRequest> {
  const tenantId = useTenantId();
  const { api } = usePorts();
  const queryClient = useQueryClient();
  return useMutation(createOptimisticMutation(queryClient, {
    mutationKey: [...settingsKeys.settings(tenantId), "scoring-guidance"],
