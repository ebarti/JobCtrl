...
import { useRestoreJobsBulkMutation } from "./useRestoreJobsBulkMutation.js";
...
describe("useRestoreJobsBulkMutation", () => {
  it("removes restored jobs from the cached list optimistically", async () => {
    const { result, queryClient } = renderHookWithProviders(() => useRestoreJobsBulkMutation());
  ...
  it("rolls back the optimistic patch when bulk-restore fails", async () => {
    server.use(
      http.post("*/v1/jobs/bulk-restore", () =>
