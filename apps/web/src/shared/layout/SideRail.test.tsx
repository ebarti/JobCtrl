...
describe("<SideRail>", () => {
  it("renders all fourteen nav links grouped under section labels", async () => {
    ...
    for (const label of ["Overview", "Pipeline", "Library", "Activity", "Setup"]) {
      expect(screen.getByText(label)).toBeInTheDocument();
    }

    const expectedLabels = NAV_GROUPS.flatMap((group) => group.items.map((item) => item.label));
  ...
  it("marks the current route active with aria-current for accent styling", async () => {
    ...
    await waitFor(() => expect(jobsLink).toHaveAttribute("aria-current", "page"));
