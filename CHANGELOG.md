...
All notable changes to JobHunter will be documented in this file.
...
### Added
- **Parallel workers for discovery/enrichment** - `jobhunter run --workers N` enables
...
- **Apply utility modes** - `--gen` (generate prompt for manual debugging), `--mark-applied`,
  `--mark-failed`, `--reset-failed` flags on `jobhunter apply`
- **Dry-run mode** - `jobhunter apply --dry-run` fills forms without clicking Submit
...
- **Config YAML not found after install** - moved `config/` into the package at
  `src/jobhunter/config/` so YAML files (employers, sites, searches) ship with `pip install`
...
- Interactive setup wizard (`jobhunter init`)
