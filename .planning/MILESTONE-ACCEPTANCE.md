...
- Post-completion QA correction: Apply Review rendered-resume fallback now presents selectable resume text as a PDF-style document page instead of a visible debug line list; verified by `corepack pnpm --filter @jobhunter/web test src/views/apply-review/ApplyReviewView.test.tsx`, `corepack pnpm web:check`, `git diff --check`, and live Playwright DOM/screenshot QA on `/apply-review`.
- Post-completion QA correction: Apply Review source attribution now reads the tailored text artifact for claim pins and the tailoring inspector while retaining the PDF artifact for the faithful rendered preview; verified by `corepack pnpm --filter @jobhunter/api test -- application-feedback.test.ts`, `corepack pnpm --filter @jobhunter/web test src/views/apply-review/ApplyReviewView.test.tsx`, `corepack pnpm api:check`, `corepack pnpm web:check`, `git diff --check`, and live Playwright QA on `/apply-review`.
- Post-completion QA correction: Apply Review now renders the audit rail from visible rendered-resume lines when rendered text exists, maps each line to matching generation provenance, separates audit metadata gaps from actual claim-risk findings, preserves `source-backed`/`grounded` status for backed lines, and scrolls the right rail to the selected rendered line; verified by `corepack pnpm --filter @jobhunter/web test src/views/apply-review/ApplyReviewView.test.tsx`, `corepack pnpm web:check`, `git diff --check`, and live Playwright QA on `/apply-review` showing 32/32 rendered-line audit rows, 0 false claim-risk tags, and line 36 scrolled into view.
- Post-completion QA correction: Apply Review now distinguishes exact bullet provenance from coarse annotated-change source-span fallback; line detail shows a source pointer and lineage precision, collapses broader source spans by default, and labels fallback rows `source span` instead of `source-backed`; verified by `corepack pnpm --filter @jobhunter/web test src/views/apply-review/ApplyReviewView.test.tsx`, `corepack pnpm web:check`, `git diff --check`, and live Playwright QA on `/apply-review` line 11.
- Post-completion QA correction: Apply Review right-rail audit cards now preview the source pointer for each rendered line and the selected-line detail no longer duplicates tailored artifact text already highlighted on the rendered resume; verified by `corepack pnpm --filter @jobhunter/web test src/views/apply-review/ApplyReviewView.test.tsx`, `corepack pnpm web:check`, `git diff --check`, and live Playwright QA on `/apply-review` line 5.
- Post-completion QA correction: Apply Review right-rail audit cards now include inline source evidence from the recorded source span, with selected cards expanding the full span in place; verified by `corepack pnpm --filter @jobhunter/web test src/views/apply-review/ApplyReviewView.test.tsx`, `corepack pnpm web:check`, `git diff --check`, and live Playwright QA on `/apply-review` line 8.
- Post-completion QA correction: Apply Review now replaces the right-side audit-row list plus lower detail panel with a single selected-line inspector that updates from the highlighted rendered resume line and keeps source evidence, pointer, lineage, transform, controls, ids, rationale, and risk metadata together; verified by `corepack pnpm --filter @jobhunter/web test src/views/apply-review/ApplyReviewView.test.tsx`, `corepack pnpm web:check`, `git diff --check`, and live Playwright QA on `/apply-review` line 10.
- Post-completion QA correction: Apply Review selected-line audit now maps unmatched rendered bullets to the nearest recorded section source span, keeps source evidence and pointer directly in the selected-line inspector, moves artifact-level grounding/risk gates outside that dynamic inspector, and removes the duplicate annotated resume changes ledger; verified by `corepack pnpm --filter @jobhunter/web test src/views/apply-review/ApplyReviewView.test.tsx`, `corepack pnpm web:check`, and live Playwright QA on `/apply-review` line 17 showing `source span`, 5 source evidence lines, 0 missing-source markers, 1 artifact-level risk panel outside the selected inspector, and no legacy annotated-change/tailoring-rationale section.
- Post-completion QA correction: Apply Review artifact-level grounding/risk now renders as a full-width Application Materials summary above the rendered resume audit surface instead of inside the line-by-line right rail; verified by `corepack pnpm --filter @jobhunter/web test src/views/apply-review/ApplyReviewView.test.tsx`, `corepack pnpm web:check`, `corepack pnpm web:build`, and live Playwright QA on `/apply-review` showing the risk region before `Rendered resume audit`, outside both rendered/line audit regions, and spanning 97% of the Application Materials pane width.
- Post-completion QA correction: Apply Review artifact-level grounding/risk summary now uses inline gate chips and collapsed finding groups with counts/previews so the panel stays compact while preserving access to detailed warnings; verified by `corepack pnpm --filter @jobhunter/web test src/views/apply-review/ApplyReviewView.test.tsx`, `corepack pnpm web:check`, `corepack pnpm web:build`, and live Playwright QA on `/apply-review` showing the panel at 89.5px tall with 4 inline metrics, 3 collapsed finding groups, and a 14px gap before the resume audit.
...
- Post-completion QA correction: legacy profile bullets now produce deterministic fallback evidence IDs when explicit `achievement_evidence` rows are absent, and the API read model backfills those IDs onto accepted artifacts that have source-span annotations or bullet provenance but empty `required_evidence_ids` / `seniority_evidence_ids`; verified by focused Python regressions, API projection/server regressions, `corepack pnpm api:check`, and live `/v1/artifacts/d44576c4042e4565a7babb856bef5660` QA showing no `profile evidence mapping` quality error.
- Audit correction: the earlier live landscape audit proved material artifact presence, not complete audit metadata. A follow-up review-queue audit found 9 stale accepted artifacts with no change annotations and no bullet provenance; those still require forced re-tailor with the patched worker to produce full generation-time audit envelopes.
