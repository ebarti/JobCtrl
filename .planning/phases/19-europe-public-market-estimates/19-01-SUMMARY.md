...
- Hardened review findings for tokenized geography detection, `European Union` phrase support, component/period compatibility, canonical public-source snapshots, non-persistence of the read-side `not_requested` marker, and SQLite state/component/period/confidence constraints for new market tables.
...
- `uv --project workers/automation run --extra dev pytest -q workers/automation/tests/test_market_compensation_estimator.py workers/automation/tests/test_market_compensation_repository.py -q` — passed, 32 tests.
