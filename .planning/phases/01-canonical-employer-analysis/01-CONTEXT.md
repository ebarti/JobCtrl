...
- **D-03:** The Phase 1 employer-analysis call runs through **three SDKs**: **Claude Agent SDK** + **Codex SDK** + **Google Antigravity (Gemini) SDK** (merge+synthesize over 3 diverse models). The Antigravity/Google leg was **wired 2026-06-09** (re-added after a brief deferral) via the real `google-antigravity` SDK — adapter `infrastructure/analysis/antigravity_analysis_adapter.py`, mirroring the working reference impl in `~/Github/mestre` (`mestre/vendor_lane/backends/antigravity_sdk.py`). The ensemble orchestrator is N-leg + partial-failure safe, so a missing Gemini key degrades to the 2 remaining legs (recorded failure), never a hard fail.
- **D-04:** Auth posture (as stated by the user):
  - Claude Agent SDK → the active Claude Code session / sign-in (already authenticated).
  - Antigravity/Google (Gemini) leg → `GEMINI_API_KEY` (preferred) then `GOOGLE_API_KEY` from the environment (per the mestre reference); a clear error is raised + the leg degrades if neither is set.
