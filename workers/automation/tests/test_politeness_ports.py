...
def test_honest_user_agent_renders_product_version_and_contact() -> None:
    ua = HonestUserAgent(product="JobHunter", version="1.2.3", contact_url="https://example.com")
    assert ua.header_value() == "JobHunter/1.2.3 (+https://example.com)"


def test_honest_user_agent_omits_empty_contact() -> None:
    ua = HonestUserAgent(product="JobHunter", version="1.2.3", contact_url=None)
    assert ua.header_value() == "JobHunter/1.2.3"
    assert HonestUserAgent(product="JobHunter", version="1.2.3", contact_url="  ").header_value() == (
        "JobHunter/1.2.3"
    )
...
def test_honest_user_agent_rejects_empty_components() -> None:
    ...
    with pytest.raises(ValueError, match="version"):
        HonestUserAgent(product="JobHunter", version="")
...
def test_politeness_decision_carries_outcome_and_user_agent() -> None:
    decision = PolitenessDecision(
        ...
        user_agent="JobHunter/0 (+https://example.com)",
