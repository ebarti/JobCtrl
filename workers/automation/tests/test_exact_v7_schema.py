...
from jobctrl.infrastructure.migrations import schema_v11
...
def test_fresh_runtime_creation_matches_the_exact_v9_manifest(tmp_path: Path) -> None:
    ...
    assert conn.execute("PRAGMA user_version").fetchone()[0] == EXACT_V11_MANIFEST.version
...
def test_worker_heartbeat_does_not_drift_the_exact_v9_schema(
    ...
    assert_exact_manifest(reopened, EXACT_V11_MANIFEST)
    close_connection(db_path)
...
def test_failed_fresh_init_removes_its_file_and_can_retry(
    ...
    db_path = tmp_path / "jobctrl.db"
    create_schema = schema_v11.create_exact_v11_schema
    ...
    monkeypatch.setattr(schema_v11, "create_exact_v11_schema", fail_after_partial_creation)
    with pytest.raises(RuntimeError, match="fixture creation failure"):
        init_db(db_path)
    ...
    monkeypatch.setattr(schema_v11, "create_exact_v11_schema", create_schema)
    conn = init_db(db_path)
    assert_exact_manifest(conn, EXACT_V11_MANIFEST)
    close_connection(db_path)
