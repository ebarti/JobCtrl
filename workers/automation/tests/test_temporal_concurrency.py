...
def test_max_parallel_discovery_families_defaults_to_sequential() -> None:
    ...
    assert max_parallel_discovery_families_from_env({"JOBHUNTER_MAX_PARALLEL_DISCOVERY_FAMILIES": ""}) == 1
    assert (
        max_parallel_discovery_families_from_env(
            {"JOBHUNTER_MAX_PARALLEL_DISCOVERY_FAMILIES": "nope"}
        )
        == 1
    )
...
def test_max_parallel_discovery_families_clamps_to_at_least_one() -> None:
    assert max_parallel_discovery_families_from_env({"JOBHUNTER_MAX_PARALLEL_DISCOVERY_FAMILIES": "3"}) == 3
    assert max_parallel_discovery_families_from_env({"JOBHUNTER_MAX_PARALLEL_DISCOVERY_FAMILIES": "0"}) == 1
    assert max_parallel_discovery_families_from_env({"JOBHUNTER_MAX_PARALLEL_DISCOVERY_FAMILIES": "-2"}) == 1
