...
:class:`~jobctrl.domain.contact.aggregate.Contact` and the supervised
:class:`~jobctrl.domain.contact.research.ContactResearchTask` roots (outreach
planner plan §4.3).
...
Invariants enforced here:
  ...
  * **INV-2 (provenance).** Every draft carries claim -> fact provenance
    (:class:`~jobctrl.domain.contact.outreach_gates.OutreachClaimProvenance`)
...
class OutreachSendLog:
    ...
    This is a RECORDED FACT, not a transport: JobCtrl never sends. ``channel``
...
class OutreachThread:
    ...
    def log_send(
        ...
        *,
        ...
        This is a *recorded fact*, not a transport — JobCtrl never sends
