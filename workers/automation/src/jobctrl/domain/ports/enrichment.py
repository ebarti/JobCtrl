...
The shared ``LlmPort`` lives in ``jobctrl.domain.ports.llm`` and is
...
from jobctrl.domain.ports.llm import LlmMessage, LlmPort, LlmRole
...
class DetailPageFetcherPort(Protocol):
    ...
    ``jobctrl.infrastructure.enrichment.playwright_fetcher``); cloud
