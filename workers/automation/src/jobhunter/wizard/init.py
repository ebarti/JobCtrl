"""JobHunter first-time setup wizard.
...
def _setup_profile() -> dict:
    ...
    console.print(Panel("[bold]Step 2: Profile[/bold]\nTell JobHunter about yourself. This powers scoring, tailoring, and auto-fill."))
...
def _setup_ai_features() -> None:
    ...
    if not Confirm.ask("Enable AI scoring and resume tailoring?", default=True):
        console.print("[dim]Discovery-only mode. You can configure AI later with [bold]jobhunter init[/bold].[/dim]")
        return
    ...
    env_lines = ["# JobHunter configuration", ""]
...
def run_wizard() -> None:
    ...
    console.print(
        Panel.fit(
            "[bold green]JobHunter Setup Wizard[/bold green]\n\n"
            "This will create your configuration at:\n"
            f"  [cyan]{APP_DIR}[/cyan]\n\n"
            "You can re-run this anytime with [bold]jobhunter init[/bold].",
            border_style="green",
        )
    )
    ...
    from jobhunter.config import get_tier, TIER_LABELS, TIER_COMMANDS
