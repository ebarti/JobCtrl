"""JobCtl configuration: paths, platform detection, user data."""
...
from jobctl.infrastructure.observability import source_validation_span

log = logging.getLogger(__name__)

APP_DIRNAME = ".jobctl"
DB_FILENAME = "jobctl.db"
_LEGACY_TOKEN = "job" + "hunter"
_LEGACY_APP_DIRNAME = f".{_LEGACY_TOKEN}"
_LEGACY_DB_FILENAME = f"{_LEGACY_TOKEN}.db"
...
def _rename_legacy_db_files(app_dir: Path) -> None:
    for suffix in ("", "-wal", "-shm"):
        legacy = app_dir / f"{_LEGACY_DB_FILENAME}{suffix}"
...
def migrate_default_workspace(home: Path | None = None) -> Path:
    """Move the default local workspace to the JobCtl directory once.

    Explicit ``JOBCTL_DIR`` disables this default-path migration.
    """
    global WORKSPACE_MIGRATION_NOTICE
    if os.environ.get("JOBCTL_DIR"):
        return Path(os.environ["JOBCTL_DIR"]).expanduser()

    root = Path.home() if home is None else home
    legacy_dir = root / _LEGACY_APP_DIRNAME
    current_dir = root / APP_DIRNAME
    lock_dir = root / ".jobctl-migration.lock"
    ...
    try:
        ...
        if current_dir.exists():
            return current_dir
        raise WorkspaceMigrationError(
            "another JobCtl workspace migration is in progress; retry after it completes"
        ) from exc
    ...
    try:
        ...
        _rename_legacy_db_files(current_dir)
...
def get_gmail_mcp_oauth_keys_path() -> Path:
    ...
    return Path(
        os.environ.get("JOBCTL_GMAIL_OAUTH_CLIENT_PATH")
...
def get_gmail_mcp_credentials_path() -> Path:
    ...
    return Path(
        os.environ.get("JOBCTL_GMAIL_TOKEN_PATH")
...
def gmail_mcp_auth_status() -> tuple[bool, str]:
    ...
    if credentials_path.exists():
        ...
        from jobctl.infrastructure.gmail.auth import GMAIL_SEND_SCOPE

        scopes = {part.strip() for part in str(token.get("scope") or "").split() if part.strip()}
        if GMAIL_SEND_SCOPE not in scopes:
            return (
                False,
                f"Gmail token at {credentials_path} is missing gmail.send scope; run jobctl gmail-auth",
            )
    ...
    return (
        False,
        "OAuth client found; run jobctl gmail-auth",
    )
...
def get_apply_timeout_seconds() -> int:
    ...
    """
    load_env()
    raw = os.environ.get("JOBCTL_APPLY_TIMEOUT_SECONDS")
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
            log.warning("Invalid JOBCTL_APPLY_TIMEOUT_SECONDS=%r; using default", raw)
        else:
            if parsed > 0:
                return parsed
            log.warning("JOBCTL_APPLY_TIMEOUT_SECONDS must be positive; using default")
...
def get_apply_max_budget_usd() -> float:
    ...
    load_env()
    raw = os.environ.get("JOBCTL_APPLY_MAX_BUDGET_USD")
    if raw:
        try:
            parsed = float(raw)
        except ValueError:
            log.warning("Invalid JOBCTL_APPLY_MAX_BUDGET_USD=%r; using default", raw)
        else:
            if parsed >= 0:
                return parsed
            log.warning("JOBCTL_APPLY_MAX_BUDGET_USD must be non-negative; using default")
    return float(DEFAULTS.get("apply_max_budget_usd", 5.00))


def load_env():
    """Load environment variables from ~/.jobctl/.env if it exists."""
...
def check_tier(required: int, feature: str) -> None:
    ...
    if required >= 2 and not any(os.environ.get(k) for k in ("GEMINI_API_KEY", "OPENAI_API_KEY", "LLM_URL")):
        missing.append(
            "LLM provider — run [bold]jobctl init[/bold] or set GEMINI_API_KEY, OPENAI_API_KEY, or LLM_URL; "
            "run [bold]jobctl doctor[/bold] for ensemble-leg auth"
        )
    if required >= 3:
        if not _has_claude_apply_runtime():
            missing.append("Claude apply runtime — install dependencies or set JOBCTL_CLAUDE_BIN")
        try:
            get_chrome_path()
        except FileNotFoundError:
