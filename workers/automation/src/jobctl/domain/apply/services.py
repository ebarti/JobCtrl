...
``jobctl.domain.apply.process_manager`` so the lifecycle wiring
...
class ApplyPromptBuilder:
    ...
    legacy ``jobctl.apply.prompt.build_prompt`` function (kept
...
def _default_mcp_config(
    ...
    *,
    ...
    launcher used.
    """
    from jobctl import config as _config
    ...
    apply_tools_env = {
        "JOBCTL_APPLY_CDP_ENDPOINT": f"http://localhost:{cdp_port}",
        "JOBCTL_APPLY_PROFILE_DB_PATH": str(_config.DB_PATH),
        "JOBCTL_APPLY_UPLOAD_DIR": upload_root,
    }
    ...
    return {
        "mcpServers": {
            ...
            "gmail": {
                ...
                "args": ["-m", "jobctl.infrastructure.gmail.mcp_server"],
                "env": {
                    "JOBCTL_GMAIL_ALLOWED_DOMAINS": ",".join(
                        _verification_sender_domains(application_url)
                    ),
                    "JOBCTL_GMAIL_AFTER_MS": str(int(time.time() * 1000)),
                    "JOBCTL_GMAIL_TO_EMAIL": str(personal.get("email") or ""),
                    "JOBCTL_GMAIL_TOKEN_PATH": str(_config.get_gmail_mcp_credentials_path()),
                    "JOBCTL_GMAIL_OAUTH_CLIENT_PATH": str(_config.get_gmail_mcp_oauth_keys_path()),
                },
            },
            "apply_tools": {
                "command": sys.executable,
                "args": ["-m", "jobctl.infrastructure.apply_tools.mcp_server"],
                "env": apply_tools_env,
            },
        },
    }
