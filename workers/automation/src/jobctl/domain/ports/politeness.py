...
``jobctl.infrastructure.network`` (P1); the fetch surfaces route through it
...
from jobctl.domain.discovery.source_registry import SourcePolicy

PROJECT_REPOSITORY_URL = "https://github.com/ebarti/JobCtl"
...
def _package_version() -> str:
    try:
        return importlib.metadata.version("jobctl")
...
def default_honest_user_agent() -> HonestUserAgent:
    """The built-in honest UA: ``JobCtl/<version> (+<repo url>)`` (D1)."""
