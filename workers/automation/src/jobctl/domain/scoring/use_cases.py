...
from jobctl.domain.materials.analysis import EmployerAnalysis
from jobctl.domain.ports.events import EventPublisher
from jobctl.domain.ports.llm import LlmMessage
from jobctl.domain.ports.scoring import (
    LlmPort,
...
from jobctl.domain.scoring.aggregate import JobScore
...
from jobctl.domain.scoring.services import ConstraintChecker, ScoreParseResult, ScoreParser
...
from jobctl.resume_profile import get_achievement_evidence, get_education_entries
...
class ScoreJobUseCase:
    ...
    def _call_llm(
        ...
        *,
        ...
        # full schema fill.
        with otel_trace.get_tracer("jobctl.scoring").start_as_current_span("scoring.score_job") as span:
            span.set_attribute("langfuse.observation.type", "span")
            span.set_attribute("jobctl.scoring.prompt_version", SCORE_PROMPT_VERSION)
            span.set_attribute("jobctl.scoring.schema_version", SCORE_SCHEMA_VERSION)
            span.set_attribute("jobctl.scoring.criteria_version", criteria.criteria_version)
            span.set_attribute("jobctl.scoring.profile_snapshot_version", profile_snapshot.version)
            span.set_attribute("jobctl.scoring.min_fit_score", criteria.min_fit_score)
            try:
                ...
                span.set_attribute("jobctl.scoring.parse.ok", False)
            ...
            span.set_attribute("jobctl.scoring.parse.ok", parsed.ok)
            span.set_attribute("jobctl.scoring.parser_warning_count", len(parsed.trace.parser_warnings))
            span.set_attribute("jobctl.scoring.eligibility", parsed.breakdown.eligibility.status)
            span.set_attribute("jobctl.scoring.hard_blocker_count", len(parsed.breakdown.eligibility.hard_blockers))
            span.set_attribute("jobctl.scoring.fit_band", parsed.breakdown.fit_band)
            span.set_attribute("jobctl.scoring.confidence", parsed.breakdown.confidence)
            if parsed.fit_score is not None:
                span.set_attribute("jobctl.scoring.fit_score", parsed.fit_score.value)
            if not parsed.ok:
