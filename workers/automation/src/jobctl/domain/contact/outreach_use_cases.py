...
from jobctl.domain.contact.value_objects import ContactAttribute
from jobctl.domain.materials.services import sanitize_text
from jobctl.domain.materials.value_objects import ArtifactStatus, JudgeVerdict
from jobctl.domain.ports.contact import ContactRepository, OutreachThreadRepository
...
def build_outreach_draft_prompt(
    ...
    *,
    ...
    return f"""You write short, truthful professional outreach messages for JobCtl.
...
class LogOutreachSendUseCase:
    ...
    JobCtl never sends: this use case writes the ``OutreachSendLog`` fact the
