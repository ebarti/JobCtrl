...
from jobctl.resume_profile import DEFAULT_WRITING_STYLE, get_tailoring_policy
from jobctl.infrastructure.materials.latex_pdf import normalize_resume_style
...
def extract_pdf_text(pdf_bytes: bytes) -> PdfTextResult:
    ...
    try:
        ...
        raise ValueError("Resume PDF import requires the pypdf package. Reinstall JobCtl dependencies.") from exc
...
def profile_from_resume_text(text: str, *, base_profile: dict[str, Any] | None = None) -> dict[str, Any]:
    """Build a canonical JobCtl profile draft from extracted resume text."""
