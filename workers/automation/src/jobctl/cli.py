"""JobCtl CLI — the main entry point."""
...
from jobctl.llm import DEFAULT_GEMINI_MODEL, DEFAULT_OPENAI_MODEL
from jobctl.pipeline import SUPPORTED_STAGE_ORDER, run_single_job
...
def _bootstrap() -> None:
    ...
    from jobctl.config import load_env, ensure_dirs
    ...
    from jobctl.infrastructure.observability import init_otel
    ...
    try:
        ...
        if _projection_subscription is None:
            from jobctl.infrastructure.events import get_default_publisher
...
def _check_tier2_stage(stage: str) -> None:
    ...
    from jobctl.config import check_tier
...
def _version_callback(value: bool) -> None:
    if value:
        console.print(f"[bold]jobctl[/bold] {__version__}")
        raise typer.Exit()
...
def _env_file_updates(path: Path, updates: dict[str, str]) -> str:
    ...
    lines = path.read_text(encoding="utf-8").splitlines() if path.exists() else ["# JobCtl configuration"]
    seen: set[str] = set()
    rendered: list[str] = []
    for line in lines:
        stripped = line.strip()
...
def _setup_toolchain_rows() -> list[tuple[str, bool, str]]:
    from jobctl.config import get_chrome_path
    from jobctl.infrastructure.preflight import check_playwright_chromium
...
def _run_workflow_spec_from_cli(spec, *, label: str) -> dict[str, Any]:
    ...
    try:
        ...
        console.print(
            ...
            f"{exc}\n"
            "Run [bold]jobctl doctor[/bold] and start the Temporal server + worker "
            "before retrying."
        )
...
def _load_telemetry_module():
    ...
    try:
        from jobctl.apply import telemetry as telemetry_mod
    except Exception:
        return None
...
def _render_live_workers_table() -> Table | None:
    ...
    try:
        from jobctl.apply.dashboard import get_worker_states
    except Exception:
        return None
...
def main(
    ...
    """JobCtl — AI-powered end-to-end job application pipeline."""
...
def init() -> None:
    ...
    from jobctl.wizard.init import run_wizard
...
def setup(
    ...
    from jobctl.config import ENV_PATH, ensure_dirs, load_env
    from jobctl.infrastructure.setup_probes import (
        ANALYSIS_LEGS_ENV,
    ...
    if not json_output:
        console.print("[bold]JobCtl Setup[/bold]")
    ...
    if not skip_browsers:
        commands.extend([
            ["corepack", "pnpm", "--filter", "@jobctl/web", "exec", "playwright", "install", "chromium"],
...
def apply(
    ...
    from jobctl.config import APP_DIR as _app_dir, check_tier
    from jobctl.database import count_ready_to_apply, get_connection
    ...
    if mark_applied:
        from jobctl.apply.launcher import mark_job
    ...
    if reset_failed:
        from jobctl.apply.launcher import reset_failed as do_reset
        count = do_reset()
    ...
    if profile is None:
        console.print(
            ...
            "Run [bold]jobctl init[/bold] to create your profile first."
        )
        raise typer.Exit(code=1)
    ...
    if not (gen and url):
        ...
        if ready == 0:
            console.print(
                ...
                "Run [bold]jobctl run enrich score tailor[/bold] to prepare applications."
            )
            raise typer.Exit(code=1)

    if gen:
        from jobctl.apply.launcher import gen_prompt
        from jobctl.config import get_apply_max_budget_usd
...
def job(
    ...
    Examples:
        jobctl job https://example.com/job/123              # tailor + apply
        jobctl job https://example.com/job/123 --tailor     # tailor only
        jobctl job https://example.com/job/123 --apply      # apply only
    """
    _bootstrap()
...
def digest(
    ...
    from jobctl.digest import acknowledge_digest, build_digest
    from jobctl.infrastructure.scoring.criteria_provider import read_min_fit_score
...
def status() -> None:
    ...
    from jobctl.database import get_stats

    stats = get_stats()

    console.print("\n[bold]JobCtl Pipeline Status[/bold]\n")
    ...
    if live_workers or recent_runs:
        ...
        console.print("\n[dim]Apply telemetry is not available yet. Run `jobctl apply` to populate it.[/dim]")
...
def rpc() -> None:
    ...
    """
    _bootstrap()
    from jobctl.infrastructure.observability import shutdown_otel
    ...
    from jobctl.infrastructure.rpc.workflow_starter import (
...
async def _reconcile_discovery_schedule(client: Any, task_queue: str) -> None:
    ...
    from jobctl.config import load_discovery_schedule_settings
    ...
    schedule_id = f"jobctl-discovery-{LOCAL_TENANT}"
...
# convention used by other JobCtl env flags (e.g. ``LANGFUSE_DISABLE``).
...
def _preflight_browsers_or_exit() -> None:
    ...
    ``JOBCTL_SKIP_BROWSER_PREFLIGHT=1`` to bypass (e.g. a worker that only
    runs non-browser activities).
    """
    import os

    skip = os.environ.get("JOBCTL_SKIP_BROWSER_PREFLIGHT", "").strip().lower()
    ...
    console.print(
        "[dim]Set JOBCTL_SKIP_BROWSER_PREFLIGHT=1 to start the worker anyway "
...
def worker(
    task_queue: Optional[str] = typer.Option(
        None,
        "--task-queue",
        help="Override the Temporal task queue (defaults to JOBCTL_TASK_QUEUE).",
    ),
) -> None:
    """Run the long-lived JobCtl Temporal worker."""
    _bootstrap()
    ...
    from jobctl.infrastructure.runtime_identity import (
        current_runtime_identity,
        write_worker_heartbeat,
    )
    queue = task_queue or JOBCTL_TASK_QUEUE

    async def _run() -> None:
        client = await get_temporal_client()
        await _reconcile_discovery_schedule(client, queue)
        from jobctl.apply.auto_apply import reconcile_auto_apply_loop
        ...
        console.print(
            f"[bold blue]JobCtl worker[/bold blue] running on task queue "
    ...
    *,
    ...
    while True:
        ...
        if temporal_client is not None:
            ...
            try:
                from jobctl.apply.auto_apply import reconcile_auto_apply_loop
                from jobctl.infrastructure.runtime_identity import current_runtime_identity
...
def _worker_heartbeat_iteration(
    ...
    *,
    ...
    from jobctl.infrastructure.runtime_identity import write_worker_heartbeat
...
def backup(
    output: Optional[Path] = typer.Option(
        ...
        help=(
            ...
            "Defaults to ~/.jobctl/backups/jobctl-<timestamp>.db."
        ),
    ),
) -> None:
    ...
    over ~/.jobctl/jobctl.db.
    """
    from jobctl.database import backup_database
...
def politeness_doctor_notices(conn, search_cfg: dict) -> list[tuple[str, str, str]]:
    ...
    """
    from jobctl.config import resolve_jobspy_boards
    from jobctl.infrastructure.network import resolve_honest_user_agent
...
def doctor() -> None:
    ...
    from jobctl.domain.tenant import LOCAL_TENANT
    from jobctl.infrastructure.profile import get_profile_repository
    from jobctl.infrastructure.apply.claude_code_cli import _claude_supports_budget_flag
    from jobctl.infrastructure.setup_probes import (
        probe_analysis_setup,
    ...
    try:
        profile = get_profile_repository().load(LOCAL_TENANT)
        if profile is None:
            results.append(("candidate profile", fail_mark, "Run 'jobctl init' to create"))
        else:
    ...
    if RESUME_PATH.exists():
        ...
        results.append(("resume.txt", fail_mark, "Run 'jobctl init' to add your resume"))

    import os

    resume_renderer = os.environ.get("JOBCTL_RESUME_RENDERER", "html_pdf").strip().lower()
    if resume_renderer in {"latex", "latex_pdf", "pdflatex"}:
        try:
            from jobctl.infrastructure.materials.latex_pdf import _find_pdflatex
            ...
            results.append(
                (
                    "resume PDF renderer",
                    fail_mark,
                    "Install TeX Live/MacTeX, set PDFLATEX_PATH, or unset JOBCTL_RESUME_RENDERER",
                )
            )
    else:
        try:
    ...
    if RESUME_TEMPLATE_PATH.exists():
        ...
        results.append(
            (
                ...
                "[dim]optional[/dim]",
                "Only needed with JOBCTL_RESUME_RENDERER=latex_pdf",
            )
        )
    ...
    try:
        ...
        if shutil.which(claude_bin) or Path(claude_bin).expanduser().exists():
            ...
            results.append(("Claude apply runtime", fail_mark, "set JOBCTL_CLAUDE_BIN or install dependencies"))
    ...
    from jobctl.infrastructure.observability import langfuse_disabled
    ...
    try:
        from jobctl.config import load_search_config as _load_search_config
        from jobctl.database import get_connection as _get_connection
    ...
    # --- Render results ---
    console.print()
    console.print("[bold]JobCtl Doctor[/bold]\n")
    ...
    # Tier summary
    from jobctl.config import get_tier, TIER_LABELS
    tier = get_tier()
...
def migrate_resume_html(
    ...
    from jobctl.database import get_connection
    from jobctl.infrastructure.materials.resume_html_migration import migrate_legacy_resume_pdfs
    from jobctl.infrastructure.projections.projection_builder import ProjectionBuilder
...
def gmail_auth(
    ...
    from jobctl.infrastructure.gmail.auth import GmailAuthError, authenticate
