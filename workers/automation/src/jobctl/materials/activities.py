...
from jobctl.domain.errors import JobCtlError, LlmTransientError, to_application_error
...
async def tailor_activity(payload: TailorActivityInput) -> TailorActivityOutput:
    ...
    from jobctl.infrastructure.temporal.run_in_activity import (
        run_blocking_with_heartbeat,
    )
    from jobctl.infrastructure.temporal.runtime_guard import assert_activity_runtime
    from jobctl.pipeline.runner import _run_stage_observed, _run_tailor
...
def _run_current_policy_tailoring(
    ...
    *,
    ...
    from jobctl.pipeline.current_policy_selectors import tailoring_current_policy_job_urls
...
def _run_selected_tailoring(
    ...
    *,
    ...
    from jobctl.scoring.tailor import tailor_job_by_url
...
async def cover_activity(payload: CoverActivityInput) -> CoverActivityOutput:
    ...
    from jobctl.infrastructure.temporal.runtime_guard import assert_activity_runtime
    from jobctl.pipeline.runner import _run_cover, _run_stage_observed
...
def _raise_on_failure(stage: str, result: dict[str, Any], error_type: type[JobCtlError]) -> None:
...
def _render_pdf_for_job(payload: RenderPdfActivityInput) -> dict[str, Any]:
    ...
    from jobctl.domain.materials.use_cases import RenderPdfUseCase
    from jobctl.domain.tenant import TenantId
