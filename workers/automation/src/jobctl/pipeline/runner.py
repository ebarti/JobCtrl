"""JobCtl pipeline stage helpers used by Temporal activities."""
...
from jobctl import database as db_module
from jobctl.database import init_db, get_connection
from jobctl.domain.discovery.scheduler import (
    DiscoveryRun,
    DiscoveryRunCounts,
...
from jobctl.domain.errors import LlmTransientError, SourceUnavailableError, TransientNetworkError
from jobctl.domain.tenant import LOCAL_TENANT
from jobctl.infrastructure.discovery.sqlite_run_repository import (
...
from jobctl.infrastructure.observability.source_spans import discovery_run_span
...
def _pipeline_tracer():
    return trace.get_tracer("jobctl.pipeline")
...
def _record_pipeline_observation_event(
    ...
    try:
        with _pipeline_tracer().start_as_current_span(f"pipeline.event.{stage}.{event_type}") as span:
            ...
            span.set_attribute("jobctl.pipeline.event_type", event_type)
            span.set_attribute("jobctl.pipeline.message", message)
...
def _set_pipeline_span_attributes(span, stage: str, *, observation_type: str = "span") -> None:  # type: ignore[no-untyped-def]
    span.set_attribute("jobctl.pipeline.stage", stage)
    span.set_attribute("jobctl.pipeline.job_id", _PIPELINE_JOB_ID)
    span.set_attribute("langfuse.trace.name", "jobctl.pipeline")
...
def _run_stage_observed(
    ...
    *,
    ...
    with _pipeline_tracer().start_as_current_span(f"pipeline.stage.{stage}") as span:
        ...
        span.set_attribute("jobctl.pipeline.mode", mode)
        span.set_attribute("jobctl.pipeline.pass_number", pass_number)
        try:
        ...
        span.set_attribute("jobctl.pipeline.status", status)
        span.set_attribute("jobctl.pipeline.duration_ms", duration_ms)
...
def _run_discovery_source(
    ...
    *,
    ...
    with (
        ...
        span.set_attribute("jobctl.pipeline.source", source)
        span.set_attribute("jobctl.discovery.run_id", run_id)
        try:
        ...
        status = "ok"
        span.set_attribute("jobctl.pipeline.source_status", status)
        span.set_attribute("jobctl.discovery.result.total", counts.total)
        span.set_attribute("jobctl.discovery.result.new_jobs", counts.new_jobs)
        _record_discovery_source_attempts(
            source,
            scheduled_sources,
    ...
    *,
    ...
    if family == "jobspy":
        ...
        def run_jobspy(run_id: str | None = None) -> dict:
            ...
            try:
                from jobctl.discovery.jobspy import run_discovery
            except ImportError:
    ...
    if family == "workday":
        ...
        def run_workday(run_id: str | None = None) -> dict:
            ...
            from jobctl.discovery.workday import run_workday_discovery
    ...
    if family == "smartextract":
        ...
        def run_smart_extract_source() -> dict:
            ...
            from jobctl.discovery.smartextract import run_smart_extract
...
def run_discovery_legacy_once(
    ...
    def finish_discovery() -> dict:
        ...
        try:
            from jobctl.pipeline.preparation import start_discovery_preparation_workflows
...
def _run_enrich(
    ...
    from jobctl.enrichment.detail import run_enrichment
...
def _run_score(
    ...
    from jobctl.scoring.scorer import run_scoring
...
def _run_tailor(
    ...
    from jobctl.scoring.tailor import run_tailoring
...
def _run_cover(
    ...
    from jobctl.database import get_jobs_by_stage
    from jobctl.scoring.cover_letter import cover_letter_by_url
...
def run_single_job(
    ...
    *,
    ...
    from jobctl.config import APP_DIR, DB_PATH
