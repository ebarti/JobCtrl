...
from jobctl.config import TAILORED_DIR
from jobctl.database import get_connection, get_jobs_by_stage
...
def _build_pdf_renderer() -> PdfRendererPort:
    renderer = os.environ.get("JOBCTL_RESUME_RENDERER", "html_pdf").strip().lower()
    ...
    if renderer and renderer not in {"html", "html_pdf", "playwright", "playwright_html"}:
        log.warning(
            "Unsupported JOBCTL_RESUME_RENDERER=%r; using html_pdf resume renderer",
            renderer,
        )
    ...
    for r in results:
        ...
        if r.get("status") in _success_statuses:
            ...
            set_stage_state(
                ...
                retryable=True,
                next_action=(
                    f"jobctl retry tailor {url} --reset-attempts"
                    if exhausted
                    else f"jobctl retry tailor {url}"
                ),
            )
            record_job_event(
                conn,
    ...
    *,
    ...
    if result.get("status") == "approved":
        ...
        set_stage_state(
            ...
            retryable=True,
            next_action=(
                f"jobctl retry tailor {job_url} --reset-attempts"
                if exhausted
                else f"jobctl retry tailor {job_url}"
            ),
            validate_transition=False,
        )
        record_job_event(
