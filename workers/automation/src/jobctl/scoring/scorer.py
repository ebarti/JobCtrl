...
from jobctl.config import RESUME_PATH
from jobctl.database import get_connection, get_jobs_by_stage, load_job_with_enrichment
...
from jobctl.domain.ports.materials import EmployerAnalysisRepository
...
from jobctl.domain.scoring.value_objects import ScoringCriteria
...
from jobctl.infrastructure.profile.factory import get_profile_repository
...
from jobctl.scoring.employer_analysis import build_analyze_use_case
    ...
    for job, outcome in results:
        ...
        if outcome.ok and outcome.score is not None:
            ...
            set_stage_state(
                ...
                retryable=True,
                next_action=f"jobctl retry score {url}",
            )
            record_job_event(
                conn,
                url,
    ...
    *,
    ...
    if outcome.ok and outcome.score is not None:
        ...
        set_stage_state(
            ...
            error_message=outcome.error or "Scoring failed",
            retryable=True,
            next_action=f"jobctl retry score {job_url}",
            validate_transition=False,
        )
        record_job_event(
            conn,
...
def _preferred_direct_score_for_repost(
    *,
    ...
    deleted_join = (
        ...
        """
        if _table_exists(conn, "jobctl_deleted_jobs")
        else ""
    )
...
from jobctl.domain.scoring.use_cases import SCORE_PROMPT  # noqa: E402,F401
