...
parses salary ranges, and stores accepted postings in the JobCtl database.
...
from jobctl.database import ensure_source_observation_tables, get_connection, init_db, resurface_deleted_job
from jobctl.domain.discovery import JobMetadata, PostingUrl, SearchStrategy, Source
from jobctl.domain.discovery.identity import normalize_observed_url
from jobctl.domain.discovery.source_registry import BROAD_BOARD_LEAD_POLICY
...
from jobctl.domain.job_content_identity import (
    content_match_basis,
...
# ``jobctl.infrastructure.network`` so the Enrichment context's
...
from jobctl.infrastructure.discovery.sqlite_repository import SqliteJobRepository
...
from jobctl.infrastructure.network.proxy import parse_proxy
...
def _patch_jobspy_linkedin_location_parser() -> None:
    ...
    if getattr(LinkedIn, "_jobctl_tolerates_unknown_countries", False):
        return
    ...
    LinkedIn._jobctl_tolerates_unknown_countries = True
...
def _find_content_duplicate_survivor(
    ...
    *,
    ...
    rows = conn.execute(
        ...
        FROM jobs j
        LEFT JOIN job_enrichments je ON je.job_url = j.url
        LEFT JOIN jobctl_deleted_jobs d
          ON d.job_url = j.url
...
def run_discovery(
    ...
    if not cfg:
        log.warning("No search configuration found. Run `jobctl init` to create one.")
