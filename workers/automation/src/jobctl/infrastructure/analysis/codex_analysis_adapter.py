...
to an isolated dir under the JobCtl runtime root (``~/.jobctl/codex_home``)
...
from jobctl.infrastructure.setup_probes import codex_auth_path, resolve_codex_binary
...
_CODEX_SCOPE = "jobctl.analysis.codex"

# Disable Codex plugins/apps so the isolated home only ever holds JobCtl's
...
# Isolated Codex home lives under the JobCtl runtime root, NOT ``~/.codex``.
_CODEX_HOME_DIRNAME = "codex_home"


def _isolated_codex_home() -> Path:
    """Return ``<JOBCTL_DIR>/codex_home`` (config imported lazily to avoid cycles)."""
    from jobctl.config import APP_DIR
...
def _load_async_codex_factory() -> AsyncCodexFactory:
    ...
    def _make() -> Any:
        ...
        # speak a different app-server protocol; JOBCTL_CODEX_BIN is the
