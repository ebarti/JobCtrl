...
from jobctl.infrastructure.rpc.workflow_starter import WorkflowCanceler
from jobctl.model_defaults import DEFAULT_PIPELINE_LLM_MODEL_SPEC
from jobctl.pipeline.preparation import (
...
def analyze_job(params: dict[str, Any]) -> dict[str, Any]:
    ...
    from jobctl.database import get_connection, load_job_with_enrichment
    from jobctl.domain.tenant import TenantId
    from jobctl.scoring.tailor import _build_analyze_use_case
...
def generate_outreach_draft(params: dict[str, Any]) -> dict[str, Any]:
    ...
    from jobctl.database import get_connection, load_job_with_enrichment
    from jobctl.domain.contact.outreach import OutreachDraftKind
    ...
    from jobctl.infrastructure.llm.llm_client import get_llm_adapter
...
def register_default_handlers(server: JsonRpcServer, *, canceler: WorkflowCanceler) -> None:
    """Wire the default JobCtl method set onto *server*."""
