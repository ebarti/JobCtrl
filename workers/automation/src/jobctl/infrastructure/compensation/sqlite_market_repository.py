...
from jobctl.database import ensure_market_compensation_tables
        ...
        for row in rows:
            ...
            observations.append(
                ReportedCompensationObservation(
                    ...
                    snapshot_version="jobctl-posted-compensation-v1",
                    sample_count=1,
...
def load_default_reported_compensation_observations(
    *,
    ...
    levels_fyi = _load_configured_provider_observations(
        ...
        access_var="JOBCTL_LEVELS_FYI_ACCESS_MODE",
        permitted_access_modes={"licensed_api", "licensed_data_feed", "enterprise_mcp"},
        required_true_vars=("JOBCTL_LEVELS_FYI_EUROPE_COVERAGE",),
        ref_vars=(
            "JOBCTL_LEVELS_FYI_OBSERVATIONS_PATH",
            "JOBCTL_LEVELS_FYI_OBSERVATIONS_JSON",
            "JOBCTL_LEVELS_FYI_DATA_FEED_PATH",
            "JOBCTL_LEVELS_FYI_OBSERVATIONS_URL",
            "JOBCTL_LEVELS_FYI_DATA_FEED_URL",
        ),
        auth_token_vars=(
            "JOBCTL_LEVELS_FYI_API_TOKEN",
            "JOBCTL_LEVELS_FYI_API_KEY",
            "JOBCTL_LEVELS_FYI_TOKEN",
        ),
        default_paths=(
            Path.home() / ".jobctl" / "compensation" / "levels_fyi.json",
            Path.home() / ".jobctl" / "compensation" / "levels_fyi.csv",
            Path.home() / ".jobctl" / "compensation" / "levels-fyi.json",
            Path.home() / ".jobctl" / "compensation" / "levels-fyi.csv",
        ),
    )
    glassdoor = _load_configured_provider_observations(
        ...
        access_var="JOBCTL_GLASSDOOR_ACCESS_MODE",
        permitted_access_modes={"partner_api", "written_permission"},
        required_true_vars=(),
        ref_vars=(
            "JOBCTL_GLASSDOOR_OBSERVATIONS_PATH",
            "JOBCTL_GLASSDOOR_OBSERVATIONS_JSON",
            "JOBCTL_GLASSDOOR_DATA_FEED_PATH",
            "JOBCTL_GLASSDOOR_OBSERVATIONS_URL",
            "JOBCTL_GLASSDOOR_DATA_FEED_URL",
        ),
        auth_token_vars=(
            "JOBCTL_GLASSDOOR_API_TOKEN",
            "JOBCTL_GLASSDOOR_API_KEY",
            "JOBCTL_GLASSDOOR_TOKEN",
        ),
        default_paths=(
            Path.home() / ".jobctl" / "compensation" / "glassdoor.json",
            Path.home() / ".jobctl" / "compensation" / "glassdoor.csv",
        ),
    )
    eurotoptech = (
