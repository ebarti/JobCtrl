...
from jobctl.infrastructure.captcha import (
    CaptchaChallenge,
    CaptchaSolveResult,
...
class ApplyToolsMcpServer:
    def __init__(
        ...
        *,
        ...
        raw_upload_dir = upload_dir or os.environ.get("JOBCTL_APPLY_UPLOAD_DIR")
        self._upload_dir = Path(raw_upload_dir).expanduser() if raw_upload_dir else None
        self._cdp_endpoint = cdp_endpoint or os.environ.get("JOBCTL_APPLY_CDP_ENDPOINT", "")
        ...
        try:
            if method == "initialize":
                result = {
                    ...
                    "capabilities": {"tools": {}},
                    "serverInfo": {"name": "jobctl-apply-tools", "version": __version__},
                }
...
def _profile_credential(kind: str) -> str:
    ...
    raw_db_path = os.environ.get("JOBCTL_APPLY_PROFILE_DB_PATH")
    if raw_db_path:
