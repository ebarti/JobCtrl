...
``jobctl.config.load_env()`` already populated from ``~/.jobctl/.env``.
...
def init_otel(*, service_name: str = "jobctl", environment: str | None = None) -> None:
    ...
    resource = Resource(
        attributes={
            ...
            "service.version": JOBCTL_VERSION,
            "deployment.environment": environment or os.environ.get("JOBCTL_ENV", "local"),
        }
    )
...
def _scrub_url_credentials(span, request) -> None:  # type: ignore[no-untyped-def]
    ...
    Belt-and-suspenders: every JobCtl call path should be passing the API
