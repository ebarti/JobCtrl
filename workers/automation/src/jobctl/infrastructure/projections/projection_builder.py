...
job_enrichments / job_artifacts / jobctl_deleted_jobs) plus the
...
from jobctl.domain.operations.projections import (
    ApplyRunProjection,
...
from jobctl.domain.ports.events import EventPublisher, Subscription
...
from jobctl.infrastructure.projections.source_quality import (
...
class ProjectionBuilder:
    ...
    :mod:`jobctl.database`, which is itself thread-local-cached).
    ...
    def _on_event(self, event: DomainEvent) -> None:
        ...
        # factory is :func:`jobctl.database.get_connection` which
    ...
    def refresh(self) -> int:
        ...
        (``get_connection`` in :mod:`jobctl.database`) and tests
    ...
    def _load_deleted_at(self, job_url: str) -> str | None:
        try:
            row = self._conn.execute(
                """
                SELECT deleted_at FROM jobctl_deleted_jobs
                WHERE job_url = ?
    ...
    def _hidden_projection_jobs(self) -> set[str]:
        ...
        # jobctl_hidden_jobs (unhidden_at IS NULL) are excluded from every
    ...
    def _rebuild_dashboard(self) -> None:
        ...
        # ``jobctl_deleted_jobs`` so the user-visible value agrees
...
def _job_lifecycle_exclusion_sql(
    ...
    if _table_exists(conn, "jobctl_hidden_jobs"):
        joins.append(
            "LEFT JOIN jobctl_hidden_jobs h "
