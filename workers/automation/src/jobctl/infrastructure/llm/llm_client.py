"""LlmAdapter — wraps the existing ``jobctl.llm.LLMClient`` behind the
``LlmPort`` protocol.
...
from jobctl.llm import LLMClient, create_client, get_client
...
class LlmAdapter:
    ...
    Wraps :class:`jobctl.llm.LLMClient` instances. The ``model`` keyword on
