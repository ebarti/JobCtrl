...
from jobctl.infrastructure.temporal.run_in_activity import set_activity_executor


@workflow.defn(name="JobCtlBootstrapNoOp")
...
# Pass the entire ``jobctl`` package through the workflow sandbox.
#
...
# live in ``jobctl.<context>.activities`` modules; without passthrough the
...
def build_worker(
    ...
    *,
    ...
    task_queue: str = JOBCTL_TASK_QUEUE,
) -> Worker:
    """Build a ``temporalio.worker.Worker`` bound to the JobCtl task queue."""
