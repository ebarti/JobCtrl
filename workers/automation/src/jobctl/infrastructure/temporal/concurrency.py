...
# effective bound is min(this, JOBCTL_MAX_CONCURRENT_ACTIVITIES) and the
...
def max_concurrent_activities_from_env(env: Mapping[str, str] | None = None) -> int:
    ...
    raw = source.get("JOBCTL_MAX_CONCURRENT_ACTIVITIES")
...
def max_parallel_discovery_families_from_env(env: Mapping[str, str] | None = None) -> int:
    source = env if env is not None else os.environ
    raw = source.get("JOBCTL_MAX_PARALLEL_DISCOVERY_FAMILIES")
    if raw is None or raw.strip() == "":
