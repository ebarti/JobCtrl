...
(() => {
  window.__jobctl_dryrun_installed = true;
  const bindingName = "jobctlDryRunBlocked";
  ...
  const block = (event, details) => {
    window.__jobctl_dryrun_blocked = true;
    report(details || {});
    if (event) {
      event.preventDefault();
      event.stopImmediatePropagation();
    }
    throw new Error("JobCtl dry-run blocked browser submission channel");
  };
  ...
  if (window.WebSocket) {
    ...
    function DryRunWebSocket(url, protocols) {
      ...
      throw new Error("JobCtl dry-run blocked WebSocket creation");
    }
...
def _run_dry_run_page_session(ws_url: str, guard: _DryRunCdpGuard) -> None:
    ...
    try:
        ...
        send("Network.enable")
        send("Runtime.enable")
        send("Runtime.addBinding", {"name": "jobctlDryRunBlocked"})
...
def _record_binding_call(message: dict, guard: _DryRunCdpGuard) -> None:
    params = message.get("params") or {}
    if params.get("name") != "jobctlDryRunBlocked":
        return
    try:
