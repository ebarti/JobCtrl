...
from jobctl.apply import prompt as prompt_mod  # noqa: F401  -- kept for back-compat imports
from jobctl.apply.chrome import (
    BASE_CDP_PORT,
    _kill_process_tree,
    cleanup_on_exit,
    kill_all_chrome,
...
from jobctl.apply.dashboard import (
    add_event,
    get_totals,
    init_worker,
    render_full,
...
from jobctl.domain.apply.services import ApplyPromptBuilder
from jobctl.domain.apply.value_objects import ApplyPrompt, ApplyRunId, new_apply_run_id
...
def _load_blocked():
    from jobctl.config import load_blocked_sites
...
def acquire_job(
    ...
    try:
        ...
        from jobctl.config import is_manual_ats
...
def mark_result(
    ...
    if status == "applied":
        ...
        set_stage_state(
            ...
            retryable=True,
            next_action=f"jobctl apply --url {url}",
            validate_transition=False,
        )
        record_job_event(
            conn,
        ...
        set_stage_state(
            ...
            next_action=f"jobctl apply --url {url}" if not permanent else None,
            validate_transition=False,
        )
        record_job_event(
            conn,
...
def gen_prompt(
    ...
    from jobctl.infrastructure.apply.claude_code_cli import _write_private_json
...
def _build_use_case():
    ...
    """
    from jobctl.domain.apply.process_manager import ApplySaga
    from jobctl.domain.apply.services import ApplyEligibilityChecker
    from jobctl.domain.apply.use_cases import SubmitApplicationUseCase
    from jobctl.infrastructure.apply import (
...
from jobctl.domain.tenant import LOCAL_TENANT  # noqa: E402
