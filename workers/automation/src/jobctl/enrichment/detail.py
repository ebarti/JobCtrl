...
The actual enrichment logic now lives in the domain layer:
  ...
  * extractors → ``jobctl.domain.enrichment.services``
  * use cases  → ``jobctl.domain.enrichment.use_cases``
  * fetcher    → ``jobctl.infrastructure.enrichment.playwright_fetcher``
  * persistence → ``jobctl.infrastructure.enrichment.sqlite_repository``

Per the no-strangler directive, this module:

  * imports ONLY from ``jobctl.domain``, ``jobctl.infrastructure``
    and ``jobctl.state`` / ``jobctl.database`` (NO discovery imports);
...
from jobctl.database import ensure_discovery_control_tables, init_db
...
from jobctl.infrastructure.network.proxy import ProxyConfig, parse_proxy
from jobctl.infrastructure.llm import get_llm_adapter
...
def _load_base_urls() -> dict[str, str | None]:
    ...
    from jobctl.config import load_base_urls
...
def _unblock_enrich_stage_if_blocked(conn: sqlite3.Connection, url: str) -> None:
    ...
    """
    from jobctl.state import record_job_event, set_stage_state
...
def _record_enrich_politeness_deferral(
    ...
    set_stage_state(
        ...
        retryable=True,
        next_action=f"jobctl retry enrich {url}",
    )
    record_job_event(
        conn,
        url,
                ...
                try:
                    ...
                    if status in ("ok", "partial"):
                        ...
                        set_stage_state(
                            ...
                            error_message=err.message,
                            retryable=retryable,
                            next_action=f"jobctl retry enrich {url}" if retryable else None,
                        )
                        record_job_event(
                            conn,
                            url,
...
def _default_linkedin_apply_resolver_factory() -> LinkedInApplyUrlResolver:
    ...
    identity rather than the honest JobCtl bot UA (owner decision D1/D3): the
