"""JobCtl database layer: schema, migrations, stats, and connection helpers.
...
from jobctl.config import DB_PATH, DEFAULTS, migrate_legacy_job_tables
...
def backup_database(
    ...
    *,
    ...
    over ``jobctl.db`` to restore. Nothing is ever deleted.
...
def _resolve_backup_destination(source: Path, output: Path | str | None) -> Path:
    ...
    return target_dir / f"jobctl-{timestamp}.db"
...
def _ensure_schema_version(conn: sqlite3.Connection) -> int:
    ...
    if current > SCHEMA_VERSION:
        raise IncompatibleSchemaVersionError(
            f"database was written by a newer JobCtl build "
            f"(schema version {current} > code schema version {SCHEMA_VERSION}); "
            f"upgrade JobCtl or restore a compatible backup ('jobctl backup')."
        )
    if current < SCHEMA_VERSION:
...
def ensure_operational_metric_tables(conn: sqlite3.Connection | None = None) -> list[str]:
    ...
    from jobctl.operational_metrics import ensure_operational_metric_tables as ensure_tables
...
def ensure_state_tables(conn: sqlite3.Connection | None = None) -> list[str]:
    ...
    from jobctl.state import reconcile_dependency_blockers
