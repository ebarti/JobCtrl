...
from jobctl.llm import SpendBudgetStatus, read_spend_budget_status
...
def _active_job_clauses(conn: sqlite3.Connection, alias: str) -> tuple[list[str], list[Any]]:
    ...
    if _table_exists(conn, "jobctl_hidden_jobs"):
        clauses.append(
            f"NOT EXISTS (SELECT 1 FROM jobctl_hidden_jobs h WHERE h.job_url = {alias}.job_id AND h.unhidden_at IS NULL)"
        )
