...
| **Missing-salary intelligence** | **Present — auditable numeric range.** JobCtrl preserves employer-posted, direct-market, and extrapolated benchmarks as separate authorities. Discover automatically refreshes missing or seven-day-old role-family, level, and country slices. A missing country can use official price levels and evidence-weighted same-company cross-country ratios; the UI exposes the exact anchor, inputs, raw factor, freshness, confidence, and review bounds. See [Compensation Evidence](user/compensation-evidence.md). | **Not present at this snapshot.** Career-Ops preserves the employer's advertised figure verbatim and never replaces it with researched data. When the posting has no advertised number, its offer flow skips detailed market rows and leaves `advertised_comp` null instead of producing a numeric missing-range estimate. [Offer analysis](https://github.com/santifer/career-ops/blob/16367add3f7291add757fc672a530294ae231bab/modes/oferta.md#L310-L346), [machine-summary boundary](https://github.com/santifer/career-ops/blob/16367add3f7291add757fc672a530294ae231bab/modes/oferta.md#L604-L605) | **BYO data only.** Its salary lookup uses a user-supplied dataset and skips salary benchmarking when none is configured; the reviewed snapshot does not evidence automatic missing-range discovery or geographic extrapolation. [Salary benchmarking](https://github.com/MadsLorentzen/ai-job-search/blob/faa479973aeaa7b8a1463112d088fdefff202961/README.md#L428-L430) |
