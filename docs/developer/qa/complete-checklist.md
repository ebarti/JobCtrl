...
| A browser acquisition bypasses the crawl-politeness gateway. For integrated Discovery, connected requests use the live extension and offline requests use guarded HTTP/anonymous Playwright. Ordinary robots denial must prevent page acquisition on the selected transport, run budget and shared host pacing must stop/defer work, and denial must fold into Enrich as `ENRICH_ROBOTS_DISALLOWED` / `robots_disallowed` rather than a generic scrape failure. JobStreaming keeps its documented invocation-level crawl-policy boundary. LinkedIn is the explicit owner-authenticated carve-out: its live signed-in extension session ignores the anonymous robots verdict but retains public-URL safety, exact-origin controls, pacing, budget, and no-submit. Every Temporal Enrich retry must retain its execution reference and skip the copied-profile pre-pass in both modes; an offline extension permits eligible retry/reset while unavailable worker readiness must still reject before reset. A robots-blocked job whose robots later allows must Unblock (`Blocked -> Pending`) and re-enrich on a later run rather than strand as `ENRICH_INTERNAL_ERROR`. | `workers/automation/tests/test_optional_extension.py`; `workers/automation/tests/test_live_browser_discovery.py`; `workers/automation/tests/test_enrichment_politeness_gate.py`; `apps/api/test/server.test.ts`; `apps/web/src/contexts/pipeline/components/StageTimeline.test.tsx`; live product-path QA from `docs/local-reliability-qa.md` |
...
| Crawl identity diverges from its transport: non-extension gateways and integrated anonymous broad-board requests must retain the configurable `<product>/<version> (+<contact>)` identity (no `Mozilla`) and resolve it at use time, while connected Discovery preserves Chrome's own user agent and evaluates ordinary robots with the browser-reported identity. `jobctrl doctor` must still disclose the configured standalone identity, active broad boards, and recent robots/rate-limit outcomes. | `workers/automation/tests/test_crawl_politeness_config.py`; `workers/automation/tests/test_browser_ua_propagation.py`; `workers/automation/tests/test_live_browser_discovery.py::test_robots_policy_is_fetched_and_cached_through_the_live_profile` |
