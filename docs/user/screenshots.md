...
The redesign uses full route workspaces for detailed work. Selecting a job,
artifact, contact, run, or activity event opens a bookmarkable page with its
facts, evidence, actions, and history together; it no longer depends on a
detached drawer. Each detail workspace has an explicit path back to its owning
list. Jobs, Artifacts, Contacts, and Runs carry their URL-backed list state into
that return path when it is present.
...
## Screenshot Asset Matrix

Every production primary route, detail route, import step, and Settings route
has one canonical desktop asset. Dense representative surfaces also have a
mobile companion. Filenames are stable so the README, docs site, and capture
workflow agree.
...
the docs-site hero. See [Documentation Screenshots](../local-development.md#documentation-screenshots)
...
Pipelines combines launch controls with the operations ledger. The Discover
tab keeps limit, internal concurrency, source, and dry-run controls; Apply adds
...
queue pollers/backlog/age/rates, read-model freshness, and the active-work
inventory. Calibrating, paused, stale, unavailable, and no-work ETA states stay
explicit.
...
Resume import is a three-route wizard: select a PDF, choose profile data,
style data, or both, then confirm the exact import. Back and cancel actions keep
the pending choice explicit before any profile mutation.
...
generator/judge choices, analysis perspectives, and AI execution policy.
Ready, unready, invalid, default, and override states remain distinct.
...
Browser & extension shows capability availability, adopted browser choices,
restrictions and reasons, and the pairing-token retrieval/copy plus confirmed
rotation flow without exposing unrelated browser data.

## Mobile Reflow

The fixed mobile captures prove that the same information remains reachable in
reading order at 390×844; they are not reduced-content mockups.
...
| **Profile** — form and baseline resume remain usable | **Browser settings** — capability and pairing facts reflow without clipping |
