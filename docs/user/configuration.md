...
Most people never need this page. JobCtl ships with working defaults, and the
...
Claude + Codex + Antigravity ensemble; `jobctl setup` and `jobctl doctor`
report those separately.

JobCtl configuration is intentionally local. Some settings live in the local
...
| Source | Purpose |
| --- | --- |
| `~/.jobctl/jobctl.db` | Candidate profile, discovery settings, preferences, tailoring controls, jobs, events, projections, and artifact metadata. |
| `~/.jobctl/.env` | Personal provider keys and runtime environment. |
| repo `.env` | Development-only overrides for the current checkout. |
| shell environment | One-off overrides for commands and CI. |
| `workers/automation/src/jobctl/config/*.yaml` | Packaged employer and site behavior registries (`employers.yaml`, `sites.yaml`). The dynamic source registry lives in SQLite. |

The development launcher loads `~/.jobctl/.env`, repo `.env`, and the optional
`JOBCTL_USER_ENV_PATH` file before starting local services.
...
`missing_profile_data:<field>` instead of guessing. `jobctl doctor` warns
...
| --- | --- | --- |
| `JOBCTL_DIR` | `~/.jobctl` | Local app directory for database, settings, artifacts, logs, browser worker state, and `.env`. |
| `JOBCTL_DB_PATH` | `$JOBCTL_DIR/jobctl.db` | TypeScript API database path. The Python worker ignores it and always uses `$JOBCTL_DIR/jobctl.db`, so overriding it desynchronizes the API from the worker — prefer `JOBCTL_DIR` to move both. |
| `JOBCTL_DASHBOARD_CONFIG_PATH` | `$JOBCTL_DIR/dashboard.json` | Settings file read and written by the TypeScript API (preferences, apply approval gate, spend budget). |
| `JOBCTL_API_HOST` | `127.0.0.1` | Local API bind host. Non-loopback hosts require explicit opt-in. |
| `JOBCTL_API_PORT` / `PORT` | `8766` | Local API port. |
| `JOBCTL_API_ALLOW_REMOTE_BIND` | unset | Set to `1`, `true`, or `yes` to allow non-loopback API binding. This can expose private local data. |
| `JOBCTL_WEB_PORT` | `5173` | Requested Vite development port. |
| `VITE_JOBCTL_API_BASE_URL` | proxied `/v1` | Browser API origin when not using the default Vite proxy. |
| `JOBCTL_TEMPORAL_DB` | `.dev/temporal/temporal.db` | Temporal (the workflow engine) dev-server SQLite history store. |
| `TEMPORAL_ADDRESS` | `localhost:7233` | Temporal server address used by the worker, CLI, and workflow-starting RPC. |
| `TEMPORAL_NAMESPACE` | `default` | Temporal namespace. |
| `JOBCTL_MAX_CONCURRENT_ACTIVITIES` | `4` | Maximum Temporal activities the local worker runs at once (shown on the Settings page). Set in the worker environment and restart the worker to apply. |
| `JOBCTL_API_SSE_POLL_MS` | `250` | API event-stream database poll interval in milliseconds. |
...
is required even when you disable the `claude` leg via `JOBCTL_ANALYSIS_LEGS`.
`jobctl setup` warns that analysis is not ready when synthesis auth is
missing, and `jobctl doctor` reports a dedicated `Claude synthesis auth` row
that stays red until Claude auth is present.

| Variable | What it does |
| --- | --- |
| `JOBCTL_ANALYSIS_LEGS` | Comma-separated enabled legs: `claude,codex,antigravity` by default. Setup writes this when you intentionally skip an unauthenticated leg so runs do not burn retries. Disabling the `claude` leg does **not** remove the Claude synthesis-auth requirement above. |
...
| `CODEX_HOME` | Codex home containing `auth.json`. Defaults to `~/.codex`; the JobCtl adapter copies this auth into isolated `~/.jobctl/codex_home`. |
| `JOBCTL_CODEX_BIN` | Explicit Codex runtime override. The default is the pinned `openai-codex-cli-bin` bundled binary. |
...
other surfaces, but the Codex SDK app-server path used by JobCtl needs
...
or run the Codex device login locally, then rerun `jobctl setup`.
...
| Variable | Default | What it does |
| --- | --- | --- |
| `JOBCTL_DISCOVERY_LLM_ROLE_FILTER` | `auto` | Uses an LLM to adjudicate loose role-title matches when an LLM provider is configured. Set `0` to force deterministic matching only. |
| `JOBCTL_DISCOVERY_ROLE_FILTER_MODEL` | configured LLM model | Optional model spec for discovery role adjudication. |
| `JOBCTL_MAX_PARALLEL_DISCOVERY_FAMILIES` | `1` | How many discovery source families (`jobspy`, `ats_api`, `workday`, `smartextract`) crawl at once. `1` (default) keeps families sequential — the safe, isolated behavior. Values `> 1` run that many source crawls concurrently to cut total discovery wall-clock; enrichment still runs once per batch (never concurrently). Read at run start and applied for the whole run; change it in the worker environment and restart the worker. **Tune conservatively:** each concurrent family may launch its own headless browser, so keep this ≤ `JOBCTL_MAX_CONCURRENT_ACTIVITIES` and mind memory (~roughly 300–600 MB per Chromium). Uncontrolled browser concurrency has historically destabilized long runs — see [Concurrency & Fan-out](../architecture/pipeline/concurrency.md) for the worker-capacity analysis before raising it. |
...
| Variable | Default | What it does |
| --- | --- | --- |
| `JOBCTL_CRAWL_UA_PRODUCT` | `JobCtl` | Product token in the outbound `User-Agent`. |
| `JOBCTL_CRAWL_UA_CONTACT` | project repo URL | Contact appended as `(+<contact>)`. Set it **empty** to drop the suffix. |

The effective identity is `<product>/<version> (+<contact>)` — for example
`JobCtl/0.3 (+https://github.com/ebarti/JobCtl)`. It **never impersonates
...
before crawling real sites** — `jobctl doctor` prints the effective value.
...
- **Broad boards** (`indeed`, `linkedin`, `glassdoor`, `zip_recruiter`) are
  fetched by `python-jobspy`, which owns its own transport — JobCtl cannot
  robots-gate or count its per-board requests, so it applies budget + pacing at
  the invocation boundary only, and `jobctl doctor` warns when they are on.
...
Outreach follow-ups are **surfaced-only reminders** — JobCtl never sends and
has no send capability. Their posture:

- **Conservative cadence defaults.** When you schedule a follow-up without picking
  a date, JobCtl suggests one **7 calendar days after the application was
...
| `JOBCTL_RESUME_RENDERER` | `html_pdf` | Set to `latex_pdf` only for the LaTeX resume compatibility renderer. |
...
| `JOBCTL_CLAUDE_BIN` | unset | Explicit apply-agent Claude runtime override. By default apply uses a system `claude` when present, then the pinned Claude Agent SDK bundled binary. |
| `JOBCTL_APPLY_TIMEOUT_SECONDS` | `900` | Per-job autonomous apply timeout. |
...
| `JOBCTL_LINKEDIN_APPLY_RESOLVER` | enabled | Set to `0` to disable authenticated LinkedIn outbound apply URL resolution. |
| `JOBCTL_LINKEDIN_APPLY_PROFILE_DIR` | `~/.jobctl/chrome-workers/linkedin-apply-url-resolver` | Dedicated Chrome profile for LinkedIn apply URL resolution. |
| `JOBCTL_LINKEDIN_APPLY_SOURCE_PROFILE_DIR` | platform Chrome profile dir | Optional source profile copied into the resolver profile on first use. |
| `JOBCTL_LINKEDIN_APPLY_CHROME_PROFILE` | browser default | Chrome profile name inside the resolver user-data directory. |
| `JOBCTL_LINKEDIN_APPLY_HEADLESS` | visible Chrome | Set to `1` to run the resolver headless. |
...
| Variable | Default | What it does |
| --- | --- | --- |
| `JOBCTL_GMAIL_DIR` | `~/.jobctl/gmail` | First-party Gmail connector auth directory. |
| `JOBCTL_GMAIL_OAUTH_CLIENT_PATH` | `$JOBCTL_GMAIL_DIR/oauth-client.json` | Google OAuth Desktop client file. |
| `JOBCTL_GMAIL_TOKEN_PATH` | `$JOBCTL_GMAIL_DIR/token.json` | Token written by `jobctl gmail-auth`. |

Authenticate with:

```bash
uv --project workers/automation run jobctl gmail-auth
uv --project workers/automation run jobctl doctor
```
...
| Variable | What it does |
| --- | --- |
| `JOBCTL_LEVELS_FYI_ACCESS_MODE` | Enables configured licensed Levels.fyi rows only when the mode permits the source. |
| `JOBCTL_LEVELS_FYI_EUROPE_COVERAGE` | Marks configured Levels.fyi evidence as Europe-capable. |
| `JOBCTL_LEVELS_FYI_OBSERVATIONS_PATH` / `JOBCTL_LEVELS_FYI_OBSERVATIONS_URL` | JSON or CSV observations feed. |
| `JOBCTL_GLASSDOOR_ACCESS_MODE` | Enables configured Glassdoor rows only when access is permitted. |
| `JOBCTL_GLASSDOOR_OBSERVATIONS_PATH` / `JOBCTL_GLASSDOOR_OBSERVATIONS_URL` | JSON or CSV observations feed. |
...
| `JOBCTL_ENV` | Environment attribute stamped on exported traces, default `local`. |
...
| Variable | What it does |
| --- | --- |
| `JOBCTL_E2E_APP_DIR` | Disposable app directory used by Playwright e2e. |
| `JOBCTL_E2E_DB_PATH` | E2E database path. |
| `JOBCTL_E2E_SETTINGS_PATH` | E2E settings path. |
| `JOBCTL_E2E_API_PORT` | E2E API port. |
| `JOBCTL_E2E_WEB_PORT` | E2E web port. |
| `JOBCTL_E2E_STUB_DISPATCH` | Routes selected dispatches through deterministic test stubs. |
| `VITE_JOBCTL_HIDE_DEVTOOLS` | Hides TanStack devtools in Vite builds used for public screenshots. |
