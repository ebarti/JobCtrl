...
- **Date:** 2026-07-13
- **Status:** Active / accepted plan; implementation has not started.
...
a terminal state and every required step for the jobs observed in that
execution has reached an accounted-for terminal state. A completed source
workflow with live per-job preparation workflows is **draining**, not complete.

Pre-existing work swept at the beginning of a Discover workflow is visible as
**Existing backlog**. It is not attributed to the new execution, does not
enter that execution's completion numerator/denominator, and does not prevent
the current execution from becoming complete. It still competes for the same
shared activity slots and therefore must influence the execution's ETA as
external contention.
...
type PipelineEta =
  ...
  | { status: "calibrating"; completedSamples: number; minimumSamples: number; asOf: string }
  | {
      status: "paused";
      reason: "worker_unavailable" | "budget_exceeded" | "blocked" | "no_dispatch";
      asOf: string;
    }
  | {
      status: "unavailable";
...
count, but their observed slot occupancy and queue-ahead effect must reduce
effective throughput or widen the range. When queue ordering/contention cannot
be bounded from current telemetry, return `calibrating` or `unavailable`
instead of an ETA that assumes all slots belong to the current cohort.
...
| New execution sweeps old pending tailor work | Old work appears only in Existing backlog; it is excluded from current completion counts but included as ETA contention |
