...
`workers/automation/src/jobhunter/discovery/workflow.py`, `DiscoverWorkflow._execute`:
...
- **Per-job preparation is already a durable, independent workflow.**
  `JobPreparationWorkflow` (`workers/automation/src/jobhunter/preparation/workflow.py`)
...
- **Fan-out already starts root workflows with deterministic, deduped ids.**
  ...
  (`workers/automation/src/jobhunter/pipeline/preparation.py`, line ~74) derives
...
- **The idempotency key is a pure function.**
  ...
  (`workers/automation/src/jobhunter/domain/preparation/work_items.py`, line ~116)
...
executes every activity; capacity is `JOBHUNTER_MAX_CONCURRENT_ACTIVITIES`
...
(`workers/automation/src/jobhunter/llm.py`, line ~143) is a **preflight**;
...
- **Score visible** signal: the first `JobScored` event for the run in
  ...
  `workers/automation/src/jobhunter/domain/events/scoring.py`).
...
- The worker is single-process with `JOBHUNTER_MAX_CONCURRENT_ACTIVITIES`
...
- **Approving user action:** none for running; the concurrency bound is
  process/env configuration (like `JOBHUNTER_MAX_CONCURRENT_ACTIVITIES`), not a
...
1. Seed a **disposable** workspace (e.g. `pnpm qa:seed -- /tmp/jobhunter-ttfs`)
   and point a full local stack at it via `JOBHUNTER_DIR` / isolated ports; confirm
...
| Anchor | Location |
| --- | --- |
| Sequential family loop; single global enrichment; end-of-run fan-out; failure folding | `workers/automation/src/jobhunter/discovery/workflow.py` (`_execute`, lines ~152–276; loop ~167; enrichment ~202; `_start_preparation_workflows` ~229/~279; fold ~239) |
| Run-level spend preflight | same file, `_check_spend` call ~99, def ~309 |
| Root-workflow fan-out via `USE_EXISTING` | `workers/automation/src/jobhunter/discovery/activities.py`, `discovery_preparation_fanout_activity` ~218 (dedup note ~226–228) |
| Deterministic id + `USE_EXISTING`; batch size 25; target derivation | `workers/automation/src/jobhunter/pipeline/preparation.py` (`start_discovery_preparation_workflows` ~74; ids ~177/~285; `USE_EXISTING` ~178/~286; `PREPARATION_CHILD_BATCH_SIZE` ~43; `derive_preparation_targets` ~61/~88) |
| Pure idempotency key | `workers/automation/src/jobhunter/domain/preparation/work_items.py`, `make_preparation_idempotency_key` ~116 |
| Per-job step order, per-job preflight, `min_score` gate | `workers/automation/src/jobhunter/preparation/workflow.py` (`PREPARATION_STEP_ORDER` ~38; `_preparation_spends`/`_check_spend` ~91/~225; tailor/cover `min_score` ~185/~206) |
| Spend ceiling preflight + defaults | `workers/automation/src/jobhunter/llm.py` (`check_spend_budget` ~143; `read_spend_budget_status` default `$25` ~124–129) |
| `JobScored` event (TTFS score signal) | `packages/domain-types/src/events/scoring.ts`; `workers/automation/src/jobhunter/domain/events/scoring.py` |
