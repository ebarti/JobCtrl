...
- No replacement of TanStack Query, Router, Form, Zustand, Radix, or shadcn.
...
- Plus Jakarta Sans for product text and JetBrains Mono for IDs, paths, logs,
  and payloads.
...
- 8px default rhythm, 5–8px control radii, stable 32/40/48px density rows.
...
| --- | --- | --- |
| `DisclosureSection` | Full-width page section with title, summary facts, help/action slot, and collapsible body | Native `details/summary` or Radix Collapsible; visible focus; `aria-expanded`; content remains in DOM when appropriate for form state |
...
| `SelectField` | Shared Radix/shadcn Select trigger, popover, label, hint, and error treatment | Full keyboard navigation, associated label, visible selected value, no custom div-only listbox |
...
| `SectionTabs` | A small semantic tab set within one complex surface | Radix Tabs; arrow-key navigation; active panel labelled by its trigger |
...
| Custom controls reduce accessibility | Existing Radix/shadcn primitives; real inputs; Storybook a11y plus final keyboard/axe gate |
