...
| 9.4 Release tagging | After the PyPI rename, publish a non-prerelease GitHub Release for the exact audited version/commit; its new release-only workflow builds and publishes the package | The rename train merged; `release-pypi.yml` rejects stale-tag commits, scans the renamed sdist/wheel, and enforces manifest/archive/tag parity before upload. Historical `publish.yml` remains disabled. | Delete the tag/release; if a bad artifact published to PyPI, yank it. Keep the new workflow disabled until the final release gates are confirmed. |
