...
### 5.3 Politeness gap to design in (does NOT exist today)
...
`SITE_DELAYS`). Because INV-3 requires respecting platform terms, this plan
**adds** (it does not merely reuse):

- a robots.txt fetch/parse/respect step before any contact-research fetch;
- a per-host rate limiter and a bounded per-run page budget (reusing the
  declarative knobs already on `SourcePolicy`:
  `max_pages_per_run`, `max_run_frequency`, `locator_max_requests_per_domain`,
  which are declared but not yet enforced at fetch time — enforce them here);
- a user-agent that identifies JobHunter and a documented crawl-delay honour.

This is called out explicitly as new work so an implementer does not assume the
fetchers are already polite.
...
| Politeness gap (no robots/rate-limit today) leads to disrespectful fetching. | Phase 2 adds robots/ToS + per-host rate limit + page budget as a hard prerequisite before any fetch (§5.3); enforce the declarative `SourcePolicy` knobs. |
