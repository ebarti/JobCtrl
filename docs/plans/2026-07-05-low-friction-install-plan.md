...
JobHunter's employer-analysis ensemble runs three coding-agent SDKs, exact-pinned in `workers/automation/pyproject.toml:48-54`:

| Leg | Packages | Adapter |
|---|---|---|
| Claude | `claude-agent-sdk==0.2.87` | `workers/automation/src/jobhunter/infrastructure/analysis/claude_analysis_adapter.py` (+ `infrastructure/materials/voice_adapter.py`) |
...
All three binaries read credential stores keyed to the user's home/keychain, so a bundled copy transparently reuses whatever the user's separately installed CLI wrote — **including Claude subscription logins, not just API keys**.

- **Claude** — resolution precedence: cloud switches (`CLAUDE_CODE_USE_BEDROCK`/`VERTEX`/`FOUNDRY`) → `ANTHROPIC_AUTH_TOKEN` → `ANTHROPIC_API_KEY` → `apiKeyHelper` → `CLAUDE_CODE_OAUTH_TOKEN` (from `claude setup-token`; Pro/Max/Team; ~1-year, inference-only) → the `/login` credential store (macOS Keychain service `"Claude Code-credentials"`; `~/.claude/.credentials.json` elsewhere; `CLAUDE_CONFIG_DIR` redirects). Anyone who has ever run `claude /login` is already authenticated for our ensemble with zero configuration.
- **Codex** — all surfaces (CLI, IDE, app-server, SDK) share `$CODEX_HOME/auth.json`, default `~/.codex/`. Our adapter already exploits this: it seeds an isolated `CODEX_HOME` (`~/.jobhunter/codex_home`) with a 0600 copy of the user's `auth.json` (`codex_analysis_adapter.py:85-97`). `OPENAI_API_KEY` / `CODEX_API_KEY` flow through the inherited environment for key-based users. Device-code login exists for headless enrollment (`codex login --device-auth`).
...
- Anthropic: third-party products may not offer claude.ai login/rate-limits without prior approval. Each user authenticating with their *own* subscription (`setup-token`) or API key on their own machine is the sanctioned pattern. Distributed products should steer to API keys.
...
2. **Detect and reuse before prompting.** Per leg: probe env vars and credential stores; only if nothing is found, offer (a) the vendor's login flow, (b) paste an API key, or (c) skip the leg. A user who already uses these tools sees zero prompts.
...
5. **Bash stays dumb; Python does the thinking.** A thin `scripts/install.sh` bootstraps uv/Node and hands off to a new `jobhunter setup` Typer command — cross-platform, unit-testable, sharing probe code with doctor.

---

## 3. Install flow (`scripts/install.sh` → `jobhunter setup`)

### Phase 0 — bootstrap (bash, minimal)
Detect OS/arch. Install/verify `uv` and Node 20+ with pnpm (corepack). Hand off to `uv --project workers/automation run jobhunter setup`.
...
|---|---|---|
| Claude | `ANTHROPIC_API_KEY` / `CLAUDE_CODE_OAUTH_TOKEN` / Bedrock-Vertex-Foundry switches; macOS Keychain `"Claude Code-credentials"`; `~/.claude/.credentials.json` (honor `CLAUDE_CONFIG_DIR`) | (a) `claude setup-token` (subscription; interactive browser) — invokable via the SDK's bundled CLI so no separate install; (b) paste `ANTHROPIC_API_KEY`; (c) skip leg |
| Codex | `~/.codex/auth.json` (honor `CODEX_HOME`); `OPENAI_API_KEY` / `CODEX_API_KEY` | (a) `codex login --device-auth` via the bundled binary (locate with `codex_cli_bin.bundled_codex_path()`); (b) paste `OPENAI_API_KEY`; (c) skip leg |
...
- Run the extended `jobhunter doctor` (S1).
...
|---|---|---|---|
| **S1** | Doctor gains real per-leg ensemble checks: SDK importable; bundled binary present + version (`claude_agent_sdk._cli_version.__cli_version__`, `codex --version`, antigravity wheel version); per-leg auth-chain probes. Fix the misleading Tier-2 "one LLM key suffices" and the Tier-3 `claude` row labeling | `workers/automation/src/jobhunter/cli.py:1742` (doctor); `config.py` tier logic (`:1214-1250`) | Already scoped as OSS spec §W2.6 — implement there; `jobhunter setup` reuses the same probe module |
| **S2** | Remove the Codex PATH-preference: stop passing `shutil.which("codex")` as `codex_bin`; always run the pinned bundled binary. Keep an explicit opt-in override env (`JOBHUNTER_CODEX_BIN`) as the sanctioned escape hatch (used by S5 fallback) | `codex_analysis_adapter.py:114-116` | Eliminates the repo's one version-skew exposure; matches SDK design and the no-compat-shims rule |
...
| **S7** | Docs + `.env.example`: add `ANTHROPIC_API_KEY`/`CLAUDE_CODE_OAUTH_TOKEN`; Codex auth-reuse note; `GEMINI_API_KEY`/`GOOGLE_API_KEY` scoped to the Antigravity leg; fix `docs/user/getting-started.md:66-76` single-key claim; README quickstart gains the one-command install. Update per the CLAUDE.md doc-ownership matrix | `.env.example`, `README.md`, `docs/user/getting-started.md`, `docs/user/configuration.md` | `docs/architecture.md:1006-1010` already states the correct auth model; user docs contradict it |
| **S8** | The `jobhunter setup` command + thin `scripts/install.sh`; shared probe module used by both setup and doctor | new `cli.py` command + `infrastructure/` probe module | Keeps bash dumb and the logic unit-testable |
...
2. **S5 strategy:** bump the Codex pin (if manylinux wheels exist upstream) vs. npm-install fallback with `JOBHUNTER_CODEX_BIN` on glibc Linux?
3. **S6:** is ADC/Vertex acceptance for the Antigravity leg worth the extra surface?
4. **Login UX:** should `jobhunter setup` *launch* the interactive vendor logins (`claude setup-token`, `codex login --device-auth`) or print the exact commands and re-probe?

---

## Appendix — research provenance

Local ground truth: installed wheels under `workers/automation/.venv/lib/python3.14/site-packages/` — `claude_agent_sdk/` (`_version.py`, `_cli_version.py`, `_bundled/claude`, `_internal/transport/subprocess_cli.py`, `_internal/session_resume.py`, `types.py:1702`), `openai_codex/client.py` + `codex_cli_bin/bin/codex`, `google/antigravity/` (`connections/local/local_connection.py`, `types.py`, `bin/localharness`), plus each `*.dist-info/METADATA`.
