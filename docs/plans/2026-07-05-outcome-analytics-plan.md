...
Turn data JobCtrl **already persists** into decision-useful, auditable
...
- **Canonical fact rows** live in `application_outcomes`
  ...
  `workers/automation/src/jobctrl/infrastructure/gmail/feedback.py:265`),
...
- **The funnel projection** `dashboard_projections.outcome_conversion_json` is
  ...
  (`workers/automation/src/jobctrl/infrastructure/projections/projection_builder.py:1653`).
...
- Domain scoring: `FIT_BANDS = (excellent, strong, plausible, stretch, poor)`,
  `fit_band_for_score` (`workers/automation/src/jobctrl/domain/scoring/value_objects.py:48,130`).
  A first-class `fit_band` column exists on `job_requirement_fit_reports`
  (`workers/automation/src/jobctrl/database.py:1392`).
...
| **Role family** | Only `ROLE_FAMILY_MARKERS` (`workers/automation/src/jobctrl/domain/compensation/market.py:190`) and `role_title_matcher.py` — **not persisted per job as a groupable dimension** | Absent | Net-new derivation + persistence (D3, owner-gated) |
...
- **Local QA path.** `pnpm --filter @jobctrl/web test`, `test-d`, `build`,
...
- **Local QA path.** `pnpm --filter @jobctrl/web test` + `web:storybook:test`;
...
- Web type-level: `pnpm --filter @jobctrl/web test-d`
- Web build: `pnpm --filter @jobctrl/web build`
- Storybook + a11y: `pnpm web:storybook:test`
- Web e2e (analytics + comparison smoke): `pnpm --filter @jobctrl/web e2e`
...
- `workers/automation/src/jobctrl/database.py` — `job_materials:1623`,
