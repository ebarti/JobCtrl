...
- **Action.** Fetch `origin/main` and tags, verify that the audited local commit
  ...
  ref `v2.0.0` and these first-release inputs:
...
- **Mandatory canonical-installer cutover.** Treat the immutable signed release
  ...
  1. Record the immutable `v2.0.0` tag/release SHA and tree as **R**. Retrieve
...
3. On that final validated `main` SHA, create and push `v2.0.0`, verify the
   remote tag and `origin/main` resolve to that same SHA, then dispatch `Release
   distribution` with `--ref v2.0.0` and the six recorded first-release inputs
...
| 9.4 Release and PyPI | Owner | Protect the release environments, create `v2.0.0` on audited `main`, then dispatch with the recorded first-release inputs and verify the signed release. |
