...
| Europe public market estimates produce precise ranges from weak evidence, use non-European or licensed sources, expose unsafe source data, write rows from API GET reads, mix ESCO taxonomy with wage observations, or change fit score, sorting, filtering, apply readiness, or apply dispatch behavior | `workers/automation/tests/test_market_compensation_estimator.py`; `workers/automation/tests/test_market_compensation_repository.py`; `apps/api/test/market-compensation-estimates.test.ts`; `apps/api/test/server.test.ts` (`market compensation boundary`) |
