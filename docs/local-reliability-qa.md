...
| First-run setup or doctor overstates vendor readiness: one general LLM key is treated as enough for the employer-analysis ensemble, Codex is marked ready without persisted `CODEX_HOME/auth.json`, a system `codex` on PATH silently overrides the pinned runtime, intentionally disabled analysis legs still burn retries or reuse stale cache keys, or apply falls back to a missing system `claude` despite the bundled SDK runtime being available | `workers/automation/tests/test_setup_probes.py`; `workers/automation/tests/test_employer_analysis_leg_config.py`; `workers/automation/tests/test_codex_home_isolation.py`; `workers/automation/tests/test_claude_code_cli_adapter.py`; `apps/api/test/install-script-contract.test.ts` |
