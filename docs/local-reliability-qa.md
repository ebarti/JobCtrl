...
| The local API drops its loopback `Host`-header allowlist (DNS-rebinding defense) or serves/opens an artifact whose resolved path escapes the JobHunter app directory | `apps/api/test/server.test.ts` (`rejects requests whose Host header is not a loopback host`; `blocks foreign-Host mutations before the handler runs`; `allows loopback Host headers with and without a port`; `refuses to open an artifact whose path resolves outside the app directory`; `refuses to preview a PDF artifact whose path resolves outside the app directory`) |
...
| Kill-worker discovery resumption regresses, requiring a reaper or manual action instead of Temporal retrying incomplete source-family activities and completing discovery | `workers/automation/tests/test_workflow_discovery.py::test_discover_workflow_runs_sources_then_enrichment_and_fanout`; `workers/automation/tests/test_workflow_discovery.py::test_discovery_preparation_fanout_activity_uses_root_workflow_fanout`; manual QA: run Discover in a disposable workspace, kill the worker mid-source activity, restart the worker, and confirm the workflow completes with no orphan reaper |
