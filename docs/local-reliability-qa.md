...
overlay; the first render matches the final artifact's generated HTML/CSS; the
"open final file" link still opens the generated PDF artifact; and line-level
JobHunter comments appear on the resume surface rather than in a separate
side-by-side audit pane. Selecting a resume line must highlight that editor
line; comments expose source text, rationale, source precision, grounding/risk
labels when provenance exists, and missing provenance as an explicit state
instead of blank space. Manual outcomes should save with a
