...
| --- | --- | --- | --- | --- |
| `DiscoverWorkflow` | Auto-resumes source-family or enrichment activity under Temporal retry; covered by `workers/automation/tests/test_workflow_discovery.py` and manual kill-worker discovery QA. | Prompt stop propagates to source-family cancel events and terminalizes discovery progress; covered by `workers/automation/tests/test_p1b_error_inversion.py`. | CLI/API return a clear workflow-start error with no in-process fallback; covered by `workers/automation/tests/test_actions.py` and `workers/automation/tests/test_jsonrpc_handlers.py`. | Reconciler marks lost open runs terminal with `WorkflowTerminated`; covered by `workers/automation/tests/test_workflow_finalize.py`. |
...
JOBCTL_DIR=/tmp/jobctl-qa pnpm api:dev
VITE_JOBCTL_API_BASE_URL=http://127.0.0.1:8766 pnpm web:dev -- --port 5173
```
...
and a throwaway `JOBCTL_DIR`, then drives a burst of `DurabilityProbeWorkflow`
...
Safety: isolated stack only (never `~/.jobctl`, never ports 7233/8233/8766/5173),
...
| Real-path first-run time-to-value regresses or is claimed from incomplete evidence: T0 must be captured before the first install command, TTFV-1 must prove the measured real scored job through both `GET /v1/jobs` and the `/jobs` fit-score badge, and TTFV-2 must prove the same measured job's real Apply Review PDF through `GET /v1/apply/review-queue`, the `/apply-review` `open final file` link, and a non-empty `/v1/artifacts/:artifactId/preview.pdf` byte stream. Synthetic data, fixtures, seeds, timing-only records, probe-only records, skipped phases, custom work commands, and CI are invalid for this gate. Owner-run cadence is pre-release only on the owner's Apple-silicon macOS reference machine: three clean runs, median TTFV-1 under 10 minutes with worst under 15 minutes, median TTFV-2 under 30 minutes with worst under 45 minutes. | `scripts/ttfv-real.mjs run` records the owner-run measurement artifact with the default tailor-only job command; `scripts/ttfv-real.mjs probe` dry-validates the API/UI/PDF probes against an already-running real-output stack; `scripts/ttfv-real.mjs summarize` rejects non-gateable records and computes the median/worst aggregate; protocol: `docs/developer/first-run-ttfv.md` |
...
| The honest crawl user-agent stops being honest or configurable — the effective UA must remain `<product>/<version> (+<contact>)` (no `Mozilla`), reflect the `JOBCTL_CRAWL_UA_PRODUCT` / `JOBCTL_CRAWL_UA_CONTACT` env overrides on every gateway **and every browser fetch surface** (each Playwright context/page UA is resolved from the gateway at call time, never an import-time constant, so an owner override reaches the browser and robots identity == fetch identity), and `jobctl doctor` must disclose the effective UA, active broad boards, and recently robots/rate-limited sources | `workers/automation/tests/test_crawl_politeness_config.py`; `workers/automation/tests/test_browser_ua_propagation.py`; `workers/automation/tests/test_fetch_surface_enforcement.py` |
...
| Worker startup and `jobctl doctor` boot without detecting a missing or garbage-collected Playwright Chromium binary (the worker fails hours into a Discover run instead of at startup), or a dev restart truncates a process log and destroys the previous run's crash traceback instead of rotating it to `<name>.log.1` | `workers/automation/tests/test_preflight.py`; `workers/automation/tests/test_doctor_playwright.py`; manual QA: with the browser removed, `jobctl doctor` reports the `playwright chromium (scraping + PDF)` check MISSING and `jobctl worker` exits non-zero before connecting to Temporal (unless `JOBCTL_SKIP_BROWSER_PREFLIGHT=1`); write a worker log via `scripts/dev`, restart, and confirm the prior run survives as `.dev/logs/worker.log.1` |
...
1. Open Dashboard and confirm the Digest panel renders the same counts as
   `uv --project workers/automation run jobctl digest --json` for the same
   local database.
2. Run `uv --project workers/automation run jobctl digest` and confirm it
   does not change `digest_state.last_acknowledged_at`.
3. Run `uv --project workers/automation run jobctl digest --acknowledge`
...
```bash
corepack pnpm --filter @jobctl/web test src/styles/token-contract.test.ts src/styles/token-contrast.test.ts
corepack pnpm web:check
corepack pnpm web:build
corepack pnpm dlx shadcn@latest info -c apps/web
corepack pnpm --filter @jobctl/web e2e -- tests/token-foundation.spec.ts
git diff --check
```
...
```bash
corepack pnpm --filter @jobctl/web test src/shared/ui/data-table.test.tsx src/shared/ui/toast.a11y.test.tsx src/shared/ui/filterable-data-grid.test.tsx src/shared/ui/table-pager.test.tsx src/shared/lib/job-description-blocks.test.ts
corepack pnpm web:check
...
The broad `corepack pnpm --filter @jobctl/web test` command may be skipped
...
```bash
JOBCTL_E2E_APP_DIR=/tmp/jobctl-route-qa \
JOBCTL_E2E_API_PORT=8878 \
JOBCTL_E2E_WEB_PORT=5275 \
corepack pnpm --filter @jobctl/web e2e -- tests/route-visual-qa.spec.ts
```
...
`jobctl score --rescore` or the score stage with `rescore: true`.
...
file" link still opens the generated PDF artifact; and line-level JobCtl
...
single JobCtl comment open beside it; comments expose source text, rationale,
...
reload the draft, reply to a JobCtl comment while keeping the source pointer
...
For apply-agent email-verification changes, use a disposable harness page and
fixture mailbox only. The page should instruct the agent to fetch and paste raw
inbox content; the expected outcome is that the agent can receive only scoped
verification codes or links from `get_verification_code`, never raw subjects,
bodies, snippets, attachments, or unrelated mailbox content.
...
(`pnpm web:storybook:test`) is the gate; `pnpm --filter @jobctl/web test`
