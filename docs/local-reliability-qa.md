...
| Discovery title filtering hard-rejects legitimate engineering/technical roles that merely contain a business-function token (account, commercial, PMO, pricing, sales) as a team/domain qualifier rather than the role head, dropping them as false negatives before verbatim/window matching and the LLM role adjudicator can weigh in | `workers/automation/tests/test_title_filter.py` |
