...
| CL-085 | Windows Credential Manager and Linux Secret Service adapters are implemented with contract coverage. Native Windows/Linux host validation remains required before promotion; mock-only evidence is not a native-platform pass. macOS retains native host validation with synthetic credentials only. | Native credential stores | Beta | repo owner | [#888](https://github.com/ebarti/JobCtrl/issues/888); [native credential QA gate](developer/qa/regression-catalog.md#native-credential-storage-and-migration) | 2026-09-22 (native Windows/Linux validation pending) |
