...
| CL-005 | The local browser extension captures the active job page (URL and visible text) over loopback and feeds it into the existing manual-capture importer, so dedupe, snapshots, quarantine, and source provenance stay identical to other user-mediated captures. This capture action is optional even though the same unpacked extension is required for integrated Discovery. | README (What It Does; Browser Extension Discovery, Capture, And Autofill) | Current | repo owner | `BR-019`, `BR-069` (requirements.md); [local TS API](local-ts-api.md) (`POST /v1/extension/captures`); `apps/api/src/server.ts` (route → `manualCaptureImporter`) | 2026-09-02 |
| CL-006 | Integrated Discovery keeps the crawl-politeness policy around the live extension transport: ordinary anonymous `robots.txt` reads use the same current Chrome profile, an inconclusive result fails closed, per-host pacing/concurrency and a run budget gate page/API tasks, and denied/rate-limited/budget outcomes remain explicit. LinkedIn detail enrichment is the owner-authenticated carve-out: the signed-in live profile does not apply the anonymous robots verdict, while public-URL safety, exact-origin controls, pacing, budget, and no-submit remain mandatory. | README (What It Does — polite fetching); Security (Crawl Politeness); Discovery (Crawl Politeness) | Current | repo owner | `BR-069`, `TR-046` (requirements.md); `workers/automation/src/jobctrl/infrastructure/discovery/live_browser.py`; `workers/automation/src/jobctrl/enrichment/detail.py`; `workers/automation/src/jobctrl/infrastructure/network/politeness.py` | 2026-09-02 |
| CL-007 | Integrated Discovery preserves Chrome's current browser user agent and uses that returned identity for robots evaluation. The configurable `JobCtrl/<version> (+<contact>)` identity remains authoritative for standalone/non-extension gateway operations, and `jobctrl doctor` prints that configured identity. | README (What It Does — polite fetching); Security (Crawl Politeness); Discovery (Runtime) | Current | repo owner | `TR-046` (requirements.md); `workers/automation/src/jobctrl/infrastructure/discovery/live_browser.py` (`LiveChromeRobotsCache`); `workers/automation/src/jobctrl/infrastructure/network/politeness.py` (`resolve_honest_user_agent`) | 2026-09-02 |
| CL-008 | A blocked fetch is recorded as a first-class outcome—robots-disallowed, rate-limited, or budget-exhausted—rather than a generic scrape error. JobStreaming retains invocation-level request accounting, but every integrated provider session uses the extension. The current Chrome profile is the one authenticated browser identity for integrated Discovery and Enrich recovery; Temporal retries skip the copied-profile pre-pass, and Settings exposes no LinkedIn profile-copy action. | Security (Crawl Politeness); Data & Safety (External Services) | Current | repo owner | `workers/automation/src/jobctrl/infrastructure/discovery/live_browser.py`; `workers/automation/src/jobctrl/enrichment/detail.py`; [security](user/security.md) (Crawl Politeness) | 2026-09-02 |
...
| CL-087 | Every job-source acquisition owned by integrated Discovery is delegated to one explicitly selected extension installation in the user's currently running Chrome profile. Pairing token presence is not readiness: Pipelines/API require that selected installation's fresh heartbeat, and there is no direct-network, Playwright, adopted-browser, or copied-profile fallback. Active work has hard timeout/cancel cleanup, bounded four-way capacity, pre-dispatch cross-origin redirect blocking, and UTF-8 byte bounds. | README; Discovery; Browser & extension Settings | Current | repo owner | `BR-069`, `TR-046` (requirements.md); [runtime](architecture/runtime.md); [local TS API](local-ts-api.md); live-profile Discovery QA in [Reliability & QA](local-reliability-qa.md) | 2026-09-02 |
...
| **CL-008** | `Current` | JobStreaming transport cannot be robots-gated per request; the claim carries the invocation-boundary exception. | `Beta` treats that exception as a load-bearing reliability limitation rather than a scope boundary. |
| **CL-087** | `Current` | The owner explicitly selected the installed extension in the user's current Chrome profile as the only integrated Discovery browser path, with no copy or fallback. | `Beta` would require the live-profile prerequisite and failure posture to carry an explicit maturity qualifier on every launch/setup surface. |
