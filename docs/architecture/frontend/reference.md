...
| **PdfExportPort** | Frontend (Hexagonal) | Browser capability that converts the currently mounted Plate resume document into a direct PDF download. It receives a DOM element, never a local path, and does not save editor state or register an artifact. |
