...
The authoritative TypeScript runtime registry in
`packages/domain-types/src/events/` currently contains **99 event types**. The
...
match the same 99-type tuple exactly. Both projection builders fold the same
camelCase payloads, including the six `Workflow*` and four `PipelineStep*`
lifecycle events.
...
- **There is no `CoverLetterFailed` event.** Cover success is
  ...
  `WorkflowFailed`.
- **`StageQueued` is not a typed domain event.** It is not in the 99-type registry.
