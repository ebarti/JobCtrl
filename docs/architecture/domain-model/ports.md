...
| `ArtifactStoragePort` | `LocalFilesystemAdapter` (writes to `~/.jobctl/tailored_resumes/`, etc.) | `S3ArtifactAdapter` (**AWS S3** with tenant-prefixed keys: `s3://jobctl-artifacts/{tenantId}/{jobId}/`; presigned URLs for browser download; lifecycle policy for cost control) |
