...
- Cut the `jobs` table over from URL primary key to `JobId`. Domain has
  `JobId` (`workers/automation/src/jobctrl/domain/identifiers.py:12`) and
  the projections expose `jobKey`, but the storage layer still uses
  `jobs.url TEXT PRIMARY KEY` (`workers/automation/src/jobctrl/database.py`)
...
- Stop projection BUILDERS from sourcing legacy nullable `jobs.*` columns.
  ...
  `workers/automation/src/jobctrl/infrastructure/projections/projection_builder.py`
...
- Index normalized scoring keywords per job. `keywords_json` is written
  and read in `workers/automation/src/jobctrl/infrastructure/scoring/sqlite_repository.py`
...
- **ESLint + dependency-boundary setup** — no ESLint config, lint script, or
  ...
  reconcile the currently direct `@jobctrl/contracts` imports in feature
...
- **Frontend ACL `JobId` is unbranded** —
  ...
  `@jobctrl/domain-types`. Intentional ACL simplification today (every
...
- **`apps/api/test/qa-seed.ts` remaining schema parity** — The seed now covers
  ...
  are `job_enrichments`, `jobctrl_deleted_jobs`, source-registry rows, and
