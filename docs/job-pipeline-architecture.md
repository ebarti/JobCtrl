...
- Applies exact-plus-recall intent to every discovery source family, but the
  ...
  internally instead of multiplying `queries x sources`. Smart Extract static
  sources are scraped once and filtered internally; query-template Smart Extract
  sources still need per-query URLs, then apply the same title/location filters
  before storage.
