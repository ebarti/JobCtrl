...
- [x] 4.1 Seed persisted comment threads from JobCtrl audit pins and risk labels when the selected material has audit data.
- [x] 4.2 Add UI for replying to a JobCtrl line comment without suppressing the original source pointer or risk label.
...
- [x] 6.2 Tag comment replies as feedback signals while preserving the original JobCtrl comment and audit context.
...
- [x] 7.4 Run the relevant verification set: `pnpm api:test`, `pnpm web:check`, `pnpm --filter @jobctrl/web test`, focused Python materials tests, `git diff --check`, and a browser smoke path.
