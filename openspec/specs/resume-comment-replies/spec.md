...
### Requirement: JobCtrl comments persist as review threads
The system SHALL represent JobCtrl-authored line comments as persisted review threads when the underlying audit data exists. Each thread MUST be anchored to the draft, base artifact, line or semantic id when known, source pin, risk label, and lifecycle state.

#### Scenario: Opening a line with an audit comment
- **WHEN** Apply Review displays a generated resume line with a JobCtrl comment
...
The system SHALL allow the user to reply to JobCtrl line comments. Replies MUST be persisted with author, timestamp, thread id, draft revision id when known, and a bounded reply body or structured decision.

#### Scenario: User explains a disputed comment
- **WHEN** the user replies to a JobCtrl comment explaining why a claim is valid
...
- **WHEN** the user responds that a JobCtrl comment is correct
...
- **WHEN** the user edits a line in a way that addresses a JobCtrl comment
