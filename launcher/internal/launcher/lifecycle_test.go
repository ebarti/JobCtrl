...
func seedLifecycleSQLitePair(t *testing.T, python, state, marker string) {
	t.Helper()
	for _, name := range []string{"jobctrl.db", "temporal.db"} {
		code := "import pathlib,sqlite3,sys; c=sqlite3.connect(sys.argv[1]); c.execute('create table t(v)'); c.execute('insert into t values (?)',(sys.argv[2],)); c.execute('PRAGMA user_version=11') if pathlib.Path(sys.argv[1]).name == 'jobctrl.db' else None; c.commit()"
